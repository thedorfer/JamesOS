...
def build_product_plans(query: str = "", *, limit: int = 6, brand_id: str = "commerce_shop") -> list[ProductPlan]:
    ...
    for niche in niches:
        ...
        compatibility = assess_compatibility(product_type, str(niche["name"]), brand_id=brand_id)
        if not compatibility["compatible"]:
            index += 1
            continue
        candidate = {**niche, "product_type": product_type, "brand_id": brand_id}
        ...
        plan = ProductPlan(
            ...
            brand_id=str(compatibility.get("brand_id", brand_id)),
            brand_name=str(compatibility.get("brand_name", "")),
            brand_voice=str(compatibility.get("brand_voice", "")),
            brand_compatibility_status=str(compatibility.get("brand_compatibility_status", compatibility["compatibility_status"])),
            brand_compatibility_reason=str(compatibility.get("brand_compatibility_reason", compatibility["compatibility_reason"])),
