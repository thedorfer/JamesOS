...
def main():
    ...
    commands.add_parser("list");show=commands.add_parser("show");show.add_argument("profile_id");commands.add_parser("plan-migrate-commerce_shop")
    apply=commands.add_parser("apply-migrate-commerce_shop");apply.add_argument("--confirm-profile-migration",action="store_true")
    ...
    if args.command=="list":result={"profiles":[item.to_dict() for item in store.list()]}
        ...
        profile=commerce_shop_migration_plan();confirmed=args.command=="apply-migrate-commerce_shop" and args.confirm_profile_migration
