...
class ApplicationShellTests(unittest.TestCase):
    def test_app_publication_confirmation_uses_guarded_executor_and_preview_writes_nothing(self):
        service=Mock();service.orchestrator.load.return_value={"stage":"awaiting_final_approval","destination":{"etsy_shop_slug":"BagholdersSupplyCo"}}
        workflow=Mock();workflow.status.return_value={"proposal_exists":True,"proposal_sha256":"proposal-sha"};workflow._state.return_value={"stage":"awaiting_final_approval"}
        executor=Mock();executor.execute.return_value={"publication_performed":True,"order_created":False,"completed_at":"now"}
        body={"csrf_token":api._COMMERCE_CREATE_CSRF,"review_identity":"review-sha","revision_number":1,"destination":"BagholdersSupplyCo"};headers={"Origin":"http://127.0.0.1:8787"}
        with patch.object(api,"CommerceCreationService",return_value=service),patch.object(api,"_application_review_identity",return_value=("review-sha",{"revision_number":1,"printify_product_id":"draft"})),patch.object(api,"CommerceWorkflow",return_value=workflow),patch.object(api,"_commerce_publication_executor",return_value=executor),patch.object(api,"_require_local"):
            client=TestClient(api.app,base_url="http://127.0.0.1:8787");preview=client.post("/app/commerce/jobs/product-review/approve-and-publish",json=body,headers=headers);confirmed=client.post("/app/commerce/jobs/product-review/approve-and-publish",json={**body,"confirmed":True},headers=headers)
        self.assertEqual(preview.status_code,200);self.assertTrue(preview.json()["confirmation_required"]);self.assertIn("external provider write",preview.json()["confirmation"]);self.assertFalse(preview.json()["publication_performed"])
        self.assertEqual(executor.execute.call_count,1);self.assertEqual(confirmed.status_code,200);self.assertFalse(confirmed.json()["order_created"]);workflow.approve.assert_called_once_with("product-review","proposal-sha",confirmed=True)

    def test_job_polling_routes_loading_to_server_rendered_review_or_diagnostics(self):
        ...
        with patch.object(api,"list_commerce_profiles",return_value=rows),patch.object(api,"selected_profile_id",return_value="bagholder-supply"),patch.object(api,"_require_local"):
            text=TestClient(api.app,base_url="http://127.0.0.1:8787").get("/app?view=commerce.loading&job_id=product-transition").text
        self.assertIn("location.replace('/app?view=commerce.review&job_id='+encodeURIComponent(selectedJob))",text)
        self.assertIn("location.replace('/app?view=commerce.diagnostics&job_id='+encodeURIComponent(selectedJob))",text)
        self.assertIn("activeView==='commerce.loading'",text)
        self.assertIn("JamesOS is preparing an unpublished draft for human review",text)
        self.assertIn("setTimeout(watchJob,1500)",text)
        self.assertIn("setTimeout(watchJob,2500)",text)
        self.assertNotIn("location.replace('/commerce/new",text)

    def test_product_review_workspace_is_job_bound_complete_and_read_only(self):
        rows=[profile("bagholder-supply",28275232,"BagholdersSupplyCo")];service=Mock();service.review_snapshot.return_value={"job_id":"product-review","ready_for_review":True,"selected_candidate_id":"prompt_compact","generation_method":"deterministic_local_typography","dimensions":[4500,5400],"brand_display_name":"Bagholder Supply Co.","printify_shop_title":"BagholderSupplyCo","printify_product_id":"draft-fixture","listing_title":"Review title","description":"Safe description","printify_package":{"verified_from_prepared_request":True,"title":"Exact Printify title","description":"Complete Printify paragraph one.\n\nComplete paragraph two.","blueprint_name":"Unisex Shirt","blueprint_id":12,"print_provider_name":"Fixture Provider","print_provider_id":29,"garment_colors":["Black"],"sizes":["S","M"],"variant_ids":[1,2],"variants":[{"id":1,"is_enabled":True,"price":2499},{"id":2,"is_enabled":True,"price":2499}],"print_placement":"front","uploaded_image_id":"upload-fixture"},"etsy_package":{"title":"Exact Etsy title","description":"Complete Etsy-ready description.","profile_id":"bagholder-supply","listing_guidance":"bagholder-supply-listing-v1","exact_phrase":"UNREALIZED LOSSES\nBUILD CHARACTER"},"mockups":[{"asset_id":"mockup-aaaaaaaaaaaa","url":"/commerce/jobs/product-review/mockups/mockup-aaaaaaaaaaaa","label":"Black","garment_color":"Black","view":"front","source":"Printify mockup"}],"tags":[f"tag {index}" for index in range(13)],"artwork_palette":["warm cream","muted market red","muted market green"],"garment_colors":["Black","Navy"],"provider_contacted":True,"workflow_timeline":["printify_draft_ready","mockups_downloaded","ready_for_review"],"publication_status":"not_published","order_status":"not_created","artwork_url":"/commerce/jobs/product-review/artwork-preview","update_existing_draft_reason":"Not safely registered"};service.orchestrator.load.return_value={"revision_number":1,"commerce_profile_id":"bagholder-supply","destination":{"printify_shop_title":"BagholderSupplyCo","printify_shop_id":28275232,"etsy_shop_slug":"BagholdersSupplyCo"}};service.orchestrator._path.return_value=Path("/tmp/jamesos-test-product-review/orchestrator-state.json")
        with patch.object(api,"list_commerce_profiles",return_value=rows),patch.object(api,"selected_profile_id",return_value="bagholder-supply"),patch.object(api,"CommerceCreationService",return_value=service),patch.object(api,"_application_review_identity",return_value=("review-sha",{"revision_number":1,"printify_product_id":"draft-fixture","title":"Exact Printify title","description":"Complete Printify paragraph one.\n\nComplete paragraph two.","tags":[f"tag {index}" for index in range(13)],"price_cents":2499,"enabled_variants":[1,2],"colors":["Black"],"sizes":["S","M"],"placement":"front","selected_artwork":"prompt_compact"})),patch.object(api,"_require_local"):
        ...
        for response in (first,second):
            self.assertEqual(response.status_code,200);self.assertIn("id='review-artwork-preview'",response.text);self.assertIn("draft-fixture",response.text);self.assertIn("Visual assets (2)",response.text);self.assertIn("Complete Printify paragraph one.\n\nComplete paragraph two.",response.text);self.assertIn("Complete Etsy-ready description.",response.text);self.assertIn("Exact persisted prepared request",response.text);self.assertIn("Prepared locally — not published to Etsy",response.text);self.assertIn("Etsy tags (13)",response.text);self.assertIn("<strong>Publication:</strong> no",response.text);self.assertIn("<strong>Order:</strong> no",response.text);self.assertIn("Update existing unpublished draft",response.text)
            self.assertIn("selectedJob?'&job_id='",response.text)
            for expected in ("Final human approval","Approve and Publish to Etsy","BagholdersSupplyCo","$24.99","All 13 tags","No customer order will be created"):
                self.assertIn(expected,response.text)
            for expected in ("Preparing publication confirmation…","Publishing…","Confirmation preview failed (HTTP ","Publication failed (HTTP ","Publication canceled. Nothing was published.","publicationBusy"):
                self.assertIn(expected,response.text)
            self.assertNotIn("publication-confirmation-copy",response.text)
    ...
    def test_actual_app_bootstrap_runs_in_browser_and_binds_safe_shell_controls(self):
        ...
        self.assertEqual(response.headers["content-security-policy"],"default-src 'none'; img-src 'self'; script-src 'unsafe-inline'; connect-src 'self'; style-src 'unsafe-inline'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'")
        ...
        prep=prep.replace("<script>window.smokeCalls=[];", "<script>window.confirm=()=>true;window.smokeCalls=[];")
        prep=prep.replace("else if(path==='/app/chat-diagnostics')", "else if(path.startsWith('/app/agency/agents/')&&method==='POST'){const v=JSON.parse(options.body);payload=v.confirmed?{changed:true,revision:1}:{confirmation_required:true,changed:false,retained:['commerce jobs','drafts','EHF records']}}else if(path==='/app/chat-diagnostics')")
        ...
        smoke=smoke.replace("let opened=0;const input=", "document.querySelector('[data-view=\"agency.home\"]').click();document.querySelector('[data-agency-tab=\"marketplace\"]').click();const marketplaceReview=!document.querySelector('[data-agency-section=\"marketplace\"]').hidden&&document.querySelector('[data-agency-section=\"marketplace\"] details').textContent.includes('Permission review');document.querySelector('[data-agency-tab=\"my-agents\"]').click();const coreProtected=!document.querySelector('[data-agent-card=\"jade\"] [data-agent-action=\"remove\"]');document.querySelector('[data-agent-card=\"administrator\"] [data-agent-action=\"disable\"]').click();await wait();const optionalDisabled=document.querySelector('[data-agent-card=\"administrator\"] strong').textContent.includes('Disabled');let opened=0;const input=")
        ...
        smoke=smoke.replace("adminSaved,merchant", "adminSaved,marketplaceReview,coreProtected,optionalDisabled,merchant")
