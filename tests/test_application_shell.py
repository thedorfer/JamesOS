...
class ApplicationShellTests(unittest.TestCase):
    def test_actual_app_bootstrap_runs_in_browser_and_binds_safe_shell_controls(self):
        ...
        for key in ("home","agency","admin","merchant","merchantVisible","commerceSafeguard","preview","shiftNoSend","enterOnce","plainHello","inert","removedExcluded","cleared","preserved","reset"):self.assertTrue(result[key],f"{key}: {result}")
    ...
    def test_realistic_ollama_prose_before_malformed_structure_is_preserved_without_repair(self):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temporary,patch("jamesos.services.ollama_service.get_config",return_value=config):
                result=WorkspaceChatService(model=ask_ollama,readiness=lambda:{"ready":True},root=Path(temporary)).message(conversation_id="conversation-12345678901234567890",message="hello",profile=rows[0],profiles=rows,workspace={"active_view":"dashboard","form":{"exact_phrase":"UNCHANGED"}})
        finally:server.shutdown();server.server_close()
        self.assertEqual(result["message"],"Hello! How can I help?");self.assertEqual(result["commands"],[]);self.assertEqual(result["warnings"],["No workspace changes were applied."]);self.assertEqual(len(requests),1);self.assertIn("format",requests[0]);provider.assert_not_called()
