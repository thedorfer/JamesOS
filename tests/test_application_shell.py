...
class ApplicationShellTests(unittest.TestCase):
    def test_actual_app_bootstrap_runs_in_browser_and_binds_safe_shell_controls(self):
        ...
        smoke=smoke.replace("document.getElementById('reset').click();const reset=", "document.getElementById('reset').click();await wait();const reset=")
        ...
        for key in ("home","agency","admin","merchant","merchantVisible","commerceSafeguard","freshTitle","manualTitle","processing","processed","preview","shiftNoSend","enterOnce","plainHello","inert","removedExcluded","cleared","preserved","reset"):self.assertTrue(result[key],f"{key}: {result}")
