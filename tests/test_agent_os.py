...
class EtsyTests(unittest.TestCase):
    ...
    def test_oauth_pkce_state_scope_single_use_and_status(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            stored=json.loads(pending.read_text());self.assertGreaterEqual(len(stored["verifier"]),43);self.assertEqual(pending.stat().st_mode&0o777,0o600)
            session=Mock();response=Mock();response.json.return_value={"access_token":"private","refresh_token":"refresh","expires_in":3600,"scope":"listings_r listings_w"};response.raise_for_status.return_value=None;session.post.return_value=response
            ...
            self.assertTrue(etsy_oauth.status(app,token,now=102)["ready_for_etsy_write"])
