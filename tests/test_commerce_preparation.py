...
class UnifiedCommercePreparationTests(unittest.TestCase):
    def test_chromium_loading_path_reaches_review_after_fake_provider_preparation(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            mocked=Mock();mocked.safe_status.return_value=status;mocked.review_snapshot.return_value={"job_id":ready["job_id"],"ready_for_review":True,"selected_candidate_id":"best","generation_method":"deterministic_local_typography","dimensions":[4500,5400],"brand_display_name":"Fixture","printify_shop_title":"Fixture","printify_product_id":"private-product","listing_title":"Market review","description":"Review description","printify_package":{"verified_from_prepared_request":True,"title":"Market review","description":"Full Printify description paragraph.\n\nSecond paragraph.","blueprint_name":"Unisex shirt","blueprint_id":12,"print_provider_name":"Fixture Provider","print_provider_id":29,"garment_colors":["Black"],"sizes":["S","M"],"variant_ids":[1,2]},"etsy_package":{"title":"Market review Etsy title","description":"Full Etsy-ready description.","profile_id":"private-profile","listing_guidance":"fixture-v1","exact_phrase":"MARKET REVIEW"},"mockups":[{"asset_id":"mockup-aaaaaaaaaaaa","url":f"/commerce/jobs/{ready['job_id']}/mockups/mockup-aaaaaaaaaaaa","label":"Black","garment_color":"Black","view":"front","source":"Printify mockup"}],"tags":[f"tag {index}" for index in range(13)],"artwork_palette":["warm cream","muted market red","muted market green"],"garment_colors":["Black"],"provider_contacted":True,"workflow_timeline":["printify_draft_ready","mockups_downloaded","ready_for_review"],"publication_status":"not_published","order_status":"not_created","artwork_url":f"/commerce/jobs/{ready['job_id']}/artwork-preview"}
            ...
            mockup_path=orchestrator.root/ready["job_id"]/"mockup.jpg";Image.new("RGB",(9,6),(20,20,20)).save(mockup_path);mockup_bytes=mockup_path.read_bytes()
            class Handler(BaseHTTPRequestHandler):
                def do_GET(self):
                    ...
                    elif "/mockups/" in self.path:body=mockup_bytes;content="image/jpeg"
            ...
            self.assertIn(f"view=commerce.review&amp;job_id={ready['job_id']}",rendered);self.assertIn("9x6 http://127.0.0.1:",rendered);self.assertIn(f"/{ready['job_id']}/mockups/mockup-aaaaaaaaaaaa",rendered);self.assertIn("Visual assets (2)",rendered);self.assertIn("Full Printify description paragraph.",rendered);self.assertIn("Full Etsy-ready description.",rendered);self.assertIn("private-product",rendered);self.assertIn("Etsy tags (13)",rendered);self.assertIn("Publication:</strong> no",rendered);self.assertIn("Order:</strong> no",rendered);self.assertEqual(orchestrator.resume_calls,1);self.assertEqual(orchestrator.review_calls,0)
