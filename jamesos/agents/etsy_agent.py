...
class EtsyAgent(AgentDefaults):
    ...
    def plan(self,request):
        action="deactivate" if request.requested_capability=="marketplace.listing.deactivate" else "verify" if request.requested_capability=="marketplace.listing.verify_state" else "read"
    ...
    def execute(self,plan,context):
        ...
        if dry:return AgentExecutionResult("planned",{"result":"etsy_state_verification_plan" if data["action"]=="verify" else "etsy_deactivation_plan","listing_id":listing_id,"etsy_listing_id":listing_id,"write_performed":False,"target_state":data["request"].get("expected_state") if data["action"]=="verify" else "inactive"})
        ...
        if data["action"] in ("read","verify"):
            expected_state=data["request"].get("expected_state")
            if expected_state and listing.get("state")!=expected_state:raise ValueError(f"Etsy listing did not reach expected {expected_state} state")
            return AgentExecutionResult("completed",{"result":"etsy_listing_final_state_verified" if data["action"]=="verify" else "etsy_listing_readiness_verified","listing_id":listing_id,"etsy_listing_id":listing_id,"shop_id":shop_id,"state":listing.get("state"),"write_performed":False},verification_status="verified")
