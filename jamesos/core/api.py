...
def application_shell_route(request:Request):
    _require_local(request);profiles=list_commerce_profiles(enabled_only=True);profile_ids={str(item.get("profile_id") or "") for item in profiles};active=str(request.query_params.get("view") or "dashboard");agency_snapshot=ShellAgencyRegistry().snapshot()
