...
def application_shell_route(request:Request):
    ...
    for agent in agency_snapshot["agents"]:
        aid=html_escape(agent["agent_id"],quote=True);workspace=agent["workspace"];open_label="Open Product Studio" if workspace=="commerce.new" else "Open Admin" if workspace=="admin.home" else "Open Scout" if workspace=="agency.book-scout" else "Open Home"
    ...
    agency_html=("<section id='agency-view' hidden><h3>The Agency</h3><nav class='agency-tabs' aria-label='Agency sections'>"+"".join(f"<button type='button' data-agency-tab='{key}'>{label}</button>" for key,label in (("my-agents","My Agents"),("marketplace","Marketplace"),("runs","Runs"),("approvals","Approvals"),("updates","Updates")))+"</nav>"
        ...
        +scout_workspace+"<section class='agency-section' id='agency-agent-detail' data-layout-locked='true' hidden><h4>Agent details</h4><div id='agency-agent-detail-content'></div><button type='button' data-agency-tab='my-agents'>Back to My Agents</button></section></section>")
