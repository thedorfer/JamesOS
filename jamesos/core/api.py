...
from jamesos.services.shell_attachments import MAX_BYTES,delete_pending_attachment,store_attachment,verify_attachments
...
def application_shell_route(request:Request):
    ...
    page=("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>JamesOS</title><style>"
        ...
        "[hidden]{display:none!important}.brand h1{display:flex;align-items:center;gap:.5rem}.health-dot{width:.85rem;height:.85rem;padding:0;border:0;border-radius:50%;background:#d49b20}.health-dot.green{background:#39d98a}.health-dot.yellow{background:#e4b13c}.health-dot.red{background:#f05b68}.health-detail{margin:.55rem 0;padding:.6rem;border:1px solid var(--line);border-radius:.55rem;background:#10131c}.health-detail[hidden]{display:none}.health-detail ul{margin:.4rem 0;padding-left:1.2rem}.context-dock{top:0;padding:.65rem 1.25rem;background:#10121a;justify-content:space-between}.dock-items,.dock-tools{display:flex;gap:.5rem;align-items:center;flex-wrap:wrap}.profile-select{width:100%;margin:.35rem 0 .65rem}.profile-guidance{color:var(--muted);font-size:.9rem}.destination-summary p{margin:.25rem 0}"
    ...
    replacements={
        ...
        "<section id='agency-view' hidden><h3>The Agency</h3><div class='layout-grid'>":"<section id='agency-view' hidden><h3>The Agency</h3><div class='layout-grid'><article class='layout-panel'><header class='panel-title'>The Merchant <span></span></header><div class='panel-content'><p>Build commerce products locally.</p><button type='button' data-view='commerce.new'>Open Product Studio</button></div></article>",
    }
    for old,new in replacements.items():page=page.replace(old,new)
    page=page.replace("undoStack=[],lastMessage='',controller=null,applyingProfile=false", "attachments=[],chatActive=false,composing=false,applyingProfile=false")
...
async def application_shell_chat_route(request:Request):
    ...
    workspace={"active_view":values.get("active_view"),"selected_job_id":values.get("selected_job_id"),"form":values.get("form") if isinstance(values.get("form"),dict) else {},"attachments":attachments}
