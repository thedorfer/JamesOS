...
@app.get("/app/activity-status")
def application_activity_status_route(request:Request):
    _require_local(request)
    return ActivityStatusService().status()
...
def application_shell_route(request:Request):
    ...
    page=("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>JamesOS</title><style>"
        ...
        "[hidden]{display:none!important}.agency-registry>.layout-panel{grid-column:auto;grid-row:auto}.brand h1{display:flex;align-items:center;gap:.5rem}.health-dot{width:.85rem;height:.85rem;padding:0;border:0;border-radius:50%;background:#d49b20}.health-dot.green{background:#39d98a}.health-dot.yellow{background:#e4b13c}.health-dot.red{background:#f05b68}.activity-indicator{display:inline-flex;align-items:center;gap:.35rem;padding:.25rem .45rem;font-size:.72rem}.activity-indicator[data-state=Working] .activity-icon{animation:activity-spin 1s linear infinite}.activity-indicator[data-state=Failed]{border-color:#f05b68}.activity-indicator[data-state='Needs attention'],.activity-indicator[data-state='Waiting for approval']{border-color:#e4b13c}@keyframes activity-spin{to{transform:rotate(360deg)}}.global-activity-panel{margin:.55rem 0;padding:.6rem;border:1px solid var(--line);border-radius:.55rem;background:#10131c;max-height:44vh;overflow:auto}.global-activity-item{border-top:1px solid var(--line);padding:.55rem 0}.global-activity-item:first-child{border-top:0}.global-activity-item p{margin:.2rem 0}.health-detail{margin:.55rem 0;padding:.6rem;border:1px solid var(--line);border-radius:.55rem;background:#10131c}.health-detail[hidden]{display:none}.health-detail ul{margin:.4rem 0;padding-left:1.2rem}.context-dock{top:0;padding:.65rem 1.25rem;background:#10121a;justify-content:space-between}.dock-items,.dock-tools{display:flex;gap:.5rem;align-items:center;flex-wrap:wrap}.profile-select{width:100%;margin:.35rem 0 .65rem}.profile-guidance{color:var(--muted);font-size:.9rem}.destination-summary p{margin:.25rem 0}"
        f"</style></head><body><div class='shell' id='app-shell'><aside class='chat' id='chat'><div class='brand'><div><h1>JamesOS <button type='button' id='health-dot' class='health-dot yellow' aria-label='System health: checking' aria-expanded='false'></button><button type='button' id='global-activity-indicator' class='activity-indicator' data-state='Idle' aria-label='Global activity: Idle' aria-expanded='false'><span id='global-activity-icon' class='activity-icon' aria-hidden='true'>○</span><span id='global-activity-label' aria-live='polite'>Idle</span></button></h1><p class='muted'>Your local workspace assistant</p></div><button type='button' id='close-chat' class='drawer-toggle'>Close</button></div><section id='health-detail' class='health-detail' aria-label='System health details' hidden><strong id='health-label'>Checking local systems…</strong><ul id='health-systems'></ul></section><section id='global-activity-panel' class='global-activity-panel' aria-label='Global JamesOS activity' aria-live='polite' hidden><strong>JamesOS activity</strong><div id='global-activity-items'><p>Idle</p></div></section><div class='transcript' id='transcript' aria-live='polite'></div><div id='confirmations' data-component='confirmation'></div><textarea id='chat-message' maxlength='2000' rows='4' placeholder='Ask JamesOS to help with this workspace'></textarea><div class='chat-actions' data-component='action_bar'><button type='button' id='send'>Send</button><button type='button' id='stop' disabled>Stop</button><button type='button' id='retry' disabled>Retry</button><button type='button' id='reset'>Reset</button></div><p id='chat-state' aria-live='polite'>Ready.</p><p id='shell-init-error' class='dock-badge warning' role='alert' hidden></p><div class='activity-actions'><strong>Activity</strong><button type='button' id='undo' disabled>Undo</button></div><div class='activity' id='activity'></div></aside><div class='divider' id='shell-divider' role='separator' aria-label='Resize chat pane' aria-orientation='vertical' tabindex='0'></div><main class='workspace'><nav class='context-dock' id='context-dock' aria-label='Context navigation'><button type='button' class='drawer-toggle' id='open-chat'>JamesOS</button><span class='dock-items' id='context-items'>{''.join(dock)}</span><span class='dock-tools'><button type='button' id='customize-layout'>Customize layout</button><span class='layout-toolbar' id='layout-toolbar' hidden><label>Theme <select id='theme-chooser'><option value='jamesos-dark'>JamesOS Dark</option></select></label><button type='button' id='save-layout'>Save</button><button type='button' id='cancel-layout'>Cancel</button><button type='button' id='reset-layout'>Reset</button></span><span id='identity'>Profile: "
        ...
        "let activityStatusTimer=null;function renderGlobalActivity(value){const indicator=q('global-activity-indicator'),label=q('global-activity-label'),icon=q('global-activity-icon'),items=q('global-activity-items');if(!indicator||!label||!icon||!items)return;indicator.dataset.state=value.state;indicator.setAttribute('aria-label','Global activity: '+value.display_label);label.textContent=value.display_label;icon.textContent=value.state==='Working'?'◌':value.state==='Waiting for approval'?'⏸':value.state==='Needs attention'?'⚠':value.state==='Failed'?'!':'○';const rows=(value.items||[]).map(item=>{const row=document.createElement('article'),title=document.createElement('strong'),details=document.createElement('p'),failure=document.createElement('p'),open=document.createElement('button');row.className='global-activity-item';row.dataset.activityState=item.state;title.textContent=item.state+': '+item.display_label;details.textContent=item.operation_state+' · elapsed '+(item.elapsed_seconds||0)+'s'+(item.progress_expected!=null?' · '+(item.progress_current||0)+'/'+item.progress_expected:'');open.type='button';open.textContent='Open workspace';open.dataset.workspaceUrl=item.workspace_url;open.onclick=()=>{location.href=item.workspace_url};row.append(title,details);if(item.safe_failure_message){failure.textContent=item.safe_failure_message;row.append(failure)}row.append(open);return row});if(!rows.length){const idle=document.createElement('p');idle.textContent='No active work, approval waits, or unresolved failures.';rows.push(idle)}items.replaceChildren(...rows)}async function pollGlobalActivity(){let delay=15000;try{const response=await fetch('/app/activity-status',{cache:'no-store'});if(!response.ok)throw new Error('HTTP '+response.status);const value=await response.json();renderGlobalActivity(value);delay=value.has_unresolved_activity?3000:Math.max(10000,Number(value.poll_interval_ms)||15000)}catch(e){renderGlobalActivity({state:'Needs attention',display_label:'Needs attention',items:[{state:'Needs attention',display_label:'Activity status unavailable',operation_state:'endpoint_unavailable',elapsed_seconds:0,workspace_url:'/app?view=diagnostics',safe_failure_message:'JamesOS could not refresh durable activity status.'}]});delay=3000}clearTimeout(activityStatusTimer);activityStatusTimer=setTimeout(pollGlobalActivity,delay)}bind('global-activity-indicator','click',()=>{const panel=q('global-activity-panel'),indicator=q('global-activity-indicator');if(!panel||!indicator)return;const open=panel.hidden;panel.hidden=!open;indicator.setAttribute('aria-expanded',String(open))});pollGlobalActivity();"
    ...
    agency_js=agency_js.replace("reconcile.textContent='Reconcile No-Submission and Allow One Retry'","reconcile.textContent=v.operation_state==='provider_submission_lost_after_restart'?'Review Lost Prompt and Allow One Retry':'Reconcile No-Submission and Allow One Retry'",1)
    retry_ui="if(['retry_authorized','remaining_samples_authorized'].includes(v.operation_state)){const retryInfo=document.createElement('div'),retryText=document.createElement('p'),retryList=document.createElement('ul'),retry=document.createElement('button');retryInfo.className='agent-card';retryText.textContent=v.operation_state==='retry_authorized'?'The earlier ComfyUI prompt was lost after the desktop restarted and no output was recovered. Only unfinished work will be retried. Completed sample outputs will never be regenerated automatically. No Amazon, Etsy, Printify, PDF, publication, purchase, or order action will occur.':'The recovered retry finished. Generate the remaining unfinished sample pages; completed outputs will not be regenerated.';(v.retry_pages||[]).forEach(page=>{const item=document.createElement('li');item.textContent=page.page_id+' — '+page.title;retryList.append(item)});retry.type='button';retry.textContent=v.operation_state==='retry_authorized'?'Retry Unfinished Sample Page':'Generate Remaining Sample Pages';retry.onclick=()=>retryUnfinishedSamples(id,retry);retryInfo.append(retryText,retryList,retry);section.append(retryInfo)}"
    agency_js=agency_js.replace("if(v.reconciliation_allowed){const reconcile",retry_ui+"if(v.reconciliation_allowed){const reconcile",1)
    agency_js+="let sampleRetryBusy=false;async function retryUnfinishedSamples(id,button){const status=q('coloring-book-status');if(sampleRetryBusy){status.textContent='An unfinished sample retry is already running. Duplicate submission blocked.';return}const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/retry';let previewResponse,preview;try{previewResponse=await samplePost(route,{confirmed:false});preview=await sampleResponseJson(previewResponse)}catch(e){status.textContent='Retry preview failed: '+(e.message||String(e));return}if(!previewResponse.ok){status.textContent='Retry preview failed (HTTP '+previewResponse.status+'): '+(preview.detail||'No retry was started.');return}if(!window.confirm(preview.confirmation)){status.textContent='Sample retry canceled. Nothing was submitted.';return}sampleRetryBusy=true;button.disabled=true;status.textContent='Retrying unfinished sample page locally…';const body={confirmed:true,project_id:preview.project_id,retry_page_ids:preview.retry_page_ids,original_lost_prompt_id:preview.original_lost_prompt_id,page_plan_revision:preview.page_plan_revision,page_plan_hash:preview.page_plan_hash,workflow_profile:preview.workflow_profile,workflow_hash:preview.workflow_hash,checkpoint:preview.checkpoint,request_id:preview.request_id,generation_identity:preview.generation_identity,comfyui_instance_identity:preview.comfyui_instance_identity,retry_attempt:preview.retry_attempt,retry_identity:preview.retry_identity};let response;try{response=await samplePost(route,body)}catch(e){status.textContent='Retry result is uncertain: '+(e.message||String(e))+'. Do not retry automatically.';return}const result=await sampleResponseJson(response);if(!response.ok){status.textContent='Sample retry failed (HTTP '+response.status+'): '+(result.detail||'No successful result was returned.');sampleRetryBusy=false;button.disabled=false;await renderProducerProject(id);return}status.textContent=(result.operation_state||result.status)+': '+(result.artifact_count||0)+' sample artifacts available.';await renderProducerProject(id);sampleRetryBusy=false}"
    ...
    agency_js += ";"
    agency_js += "const pagePlanProducerRender=renderProducerProject;renderProducerProject=async id=>{await pagePlanProducerRender(id);const r=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),v=await r.json(),out=q('coloring-book-project'),section=document.createElement('section'),h=document.createElement('h5'),state=document.createElement('p');h.textContent='Sample Page Review';state.textContent=v.status==='not_generated'&&v.project_status==='page_plan_approved'?'Page plan approved locally. No images have been generated yet.':'Sample review status: '+v.status;section.append(h,state);if(!v.provider_readiness.configured){const warning=document.createElement('p');warning.className='dock-badge warning';warning.textContent='Local image provider is not configured · reachable: '+v.provider_readiness.reachable+' · workflow: '+v.provider_readiness.workflow_configured+' · checkpoint: '+v.provider_readiness.checkpoint_configured;section.append(warning)}if(v.project_status==='page_plan_approved'&&v.status==='not_generated'){const generate=document.createElement('button');generate.type='button';generate.textContent='Generate 3 Sample Pages';generate.onclick=()=>generateSamples(id);section.append(generate)}for(const item of v.artifacts||[]){const card=document.createElement('article'),img=document.createElement('img'),title=document.createElement('strong'),details=document.createElement('details'),summary=document.createElement('summary'),pre=document.createElement('pre');card.className='agent-card';img.src='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(item.asset_id);img.alt='Generated sample for '+item.page_id;img.style.maxWidth='260px';title.textContent=item.page_id+' · '+item.review_state;summary.textContent='Prompt and technical details';pre.textContent=JSON.stringify({prompt_id:item.prompt_id,provider:item.provider_id,workflow_hash:item.workflow_hash,checkpoint:item.model_checkpoint,seed:item.seed,dimensions:[item.width,item.height],sha256:item.file_sha256,validation:item.technical_validation},null,2);details.append(summary,pre);card.append(title,img,details);for(const [label,action] of [['Approve','approve'],['Reject','reject']]){const b=document.createElement('button');b.type='button';b.textContent=label;b.onclick=()=>reviewSample(id,item.asset_id,action);card.append(b)}const regen=document.createElement('button');regen.type='button';regen.textContent='Regenerate';regen.onclick=()=>regenerateSample(id,item.asset_id);card.append(regen);section.append(card)}if((v.artifacts||[]).length){const approve=document.createElement('button');approve.type='button';approve.textContent='Approve Sample Style Locally';approve.onclick=()=>approveSampleStyle(id);section.append(approve)}out.append(section)};async function samplePost(route,body){return fetch(route,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({csrf_token:csrf,...body})})}async function generateSamples(id){const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/generate',p=await samplePost(route,{confirmed:false}),v=await p.json();if(!p.ok){q('coloring-book-status').textContent=v.detail;return}if(!v.provider_readiness.configured){q('coloring-book-status').textContent='Local image provider is not configured.';return}if(!window.confirm('Project '+v.project_id+' · plan revision '+v.page_plan_revision+' · '+v.page_plan_hash+' · samples '+v.selected_sample_page_ids.join(', ')+' · workflow '+v.workflow+' · checkpoint '+v.checkpoint+' · local GPU effects only · output '+v.output_directory+' · no PDF, Amazon upload, or publication.'))return;const r=await samplePost(route,{confirmed:true});q('coloring-book-status').textContent=r.ok?'Three local sample pages generated for review.':'Sample generation failed safely.';if(r.ok)renderProducerProject(id)}async function reviewSample(id,asset,action){const r=await samplePost('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/review',{action});q('coloring-book-status').textContent=r.ok?'Sample review saved locally.':'Review was not saved.';if(r.ok)renderProducerProject(id)}async function regenerateSample(id,asset){const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/regenerate',p=await samplePost(route,{confirmed:false}),v=await p.json();if(!p.ok||!window.confirm(v.confirmation))return;const r=await samplePost(route,{confirmed:true});q('coloring-book-status').textContent=r.ok?'Replacement sample generated locally.':'Regeneration failed safely.';if(r.ok)renderProducerProject(id)}async function approveSampleStyle(id){const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/approve-style',p=await samplePost(route,{confirmed:false}),v=await p.json();if(!p.ok){q('coloring-book-status').textContent=v.detail;return}if(!window.confirm(v.confirmation))return;const r=await samplePost(route,{confirmed:true}),done=await r.json();q('coloring-book-status').textContent=r.ok?done.message:done.detail;if(r.ok){loadColoringBookProjects();renderProducerProject(id)}};"
    agency_js += "const promptEditorRender=renderProducerProject;renderProducerProject=async id=>{await promptEditorRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),sections=[...q('coloring-book-project').querySelectorAll('section')],section=sections[sections.length-1],cards=[...section.querySelectorAll('article.agent-card')];cards.forEach((card,index)=>{const item=(samples.artifacts||[])[index],details=item&&item.prompt_details;if(!details)return;const old=card.querySelector('button:last-of-type');if(old&&old.textContent==='Regenerate')old.remove();const edit=document.createElement('button'),form=document.createElement('div'),positive=document.createElement('textarea'),negative=document.createElement('textarea'),save=document.createElement('button');edit.type='button';edit.textContent='Edit Prompt';positive.value=details.positive_prompt||'';positive.rows=7;positive.setAttribute('aria-label','Positive prompt for '+item.page_id);negative.value=details.negative_prompt||'';negative.rows=5;negative.setAttribute('aria-label','Negative prompt for '+item.page_id);save.type='button';save.textContent='Save Prompt Override Locally';form.hidden=true;form.append('Positive prompt',positive,'Negative prompt',negative,save);edit.onclick=()=>{form.hidden=!form.hidden};save.onclick=()=>saveSamplePrompt(id,item.page_id,positive.value,negative.value,save);card.append(edit,form);if(details.override_saved){const regenerate=document.createElement('button');regenerate.type='button';regenerate.textContent='Regenerate With Updated Prompt';regenerate.onclick=()=>regenerateUpdatedPrompt(id,item.asset_id,regenerate);card.append(regenerate)}})};async function saveSamplePrompt(id,page,positive_prompt,negative_prompt,button){button.disabled=true;try{const r=await samplePost('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/pages/'+encodeURIComponent(page)+'/prompt',{positive_prompt,negative_prompt}),v=await r.json();if(!r.ok)throw new Error(v.detail||'Prompt override was not saved.');q('coloring-book-status').textContent='Prompt override saved locally. No image was generated.';await renderProducerProject(id)}catch(error){q('coloring-book-status').textContent='Prompt save failed: '+error.message;button.disabled=false}}async function regenerateUpdatedPrompt(id,asset,button){if(button.disabled)return;const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/regenerate-with-updated-prompt';try{const p=await samplePost(route,{confirmed:false}),v=await p.json();if(!p.ok)throw new Error(v.detail||'Preview failed.');if(!window.confirm(v.confirmation))return;button.disabled=true;q('coloring-book-status').textContent='Regenerating '+v.page_id+' locally with the updated prompt…';const r=await samplePost(route,{confirmed:true,...v}),done=await r.json();if(!r.ok)throw new Error(done.detail||'Updated-prompt regeneration failed safely.');q('coloring-book-status').textContent='Updated-prompt sample generated locally for review.';await renderProducerProject(id)}catch(error){q('coloring-book-status').textContent='Updated-prompt regeneration failed: '+error.message;button.disabled=false}}"
    agency_js += "let sampleProgressTimer=null;const sampleActiveStates=new Set(['previewed','submission_started','provider_submitted','running','outputs_received']);function sampleProgressMessage(state){return ({previewed:'Preparing local generation…',submission_started:'Preparing local generation…',provider_submitted:'Submitted to ComfyUI…',running:'Generating locally…',outputs_received:'Processing output…',review_ready:'Ready for review',failed:'Failed safely',retry_authorized:'Retry authorization required',provider_submission_lost_after_restart:'Retry authorization required',sample_style_approved:'Ready for review'})[state]||'Sample generation status available'}function elapsedLabel(seconds){seconds=Math.max(0,Number(seconds)||0);const minutes=Math.floor(seconds/60),rest=seconds%60;return minutes?minutes+'m '+rest+'s':rest+'s'}const sampleProgressRender=renderProducerProject;renderProducerProject=async id=>{if(sampleProgressTimer){clearTimeout(sampleProgressTimer);sampleProgressTimer=null}await sampleProgressRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),progress=samples.progress||{},root=q('coloring-book-project'),sections=[...root.querySelectorAll('section')],review=sections[sections.length-1],panel=document.createElement('aside'),heading=document.createElement('h5'),message=document.createElement('strong'),refresh=document.createElement('button'),identity=progress.comfyui_instance_identity||{};panel.className='agent-card sample-generation-status';panel.dataset.operationState=progress.operation_state||'not_generated';heading.textContent='Sample Generation Status';message.textContent=progress.operation_type==='regenerate_single_page'&&progress.operation_state==='review_ready'?'New page-001 candidate ready for review.':sampleProgressMessage(progress.operation_state);refresh.type='button';refresh.textContent='Refresh Status';refresh.onclick=()=>renderProducerProject(id);panel.append(heading,message,scoutText('Operation type',progress.operation_type||'none'),scoutText('Operation state',progress.operation_state||'not_generated'),scoutText('Pages currently being generated',(progress.page_ids||[]).join(', ')||'none'),scoutText('Source artifact',progress.source_artifact_id||'none'),scoutText('Prompt revisions',(progress.old_prompt_revision??'unknown')+' → '+(progress.new_prompt_revision??'unknown')),scoutText('Prompt hashes',(progress.old_prompt_hash||'unknown')+' → '+(progress.new_prompt_hash||'unknown')),scoutText('Profiles',(progress.old_profile_id||'unknown')+' → '+(progress.new_profile_id||'unknown')),scoutText('Submitted ComfyUI prompt IDs',(progress.submitted_prompt_ids||[]).join(', ')||'none'),scoutText('Started',progress.started_at||'not started'),scoutText('Elapsed',elapsedLabel(progress.elapsed_seconds)),scoutText('ComfyUI instance',identity.instance_id?identity.instance_id+' · '+(identity.process_started_at||'start unknown'):'unavailable'),scoutText('Queue confirmation',progress.queue_confirmation_state||'unknown'),scoutText('Artifacts',(progress.artifact_count??0)+' total; '+(progress.operation_artifact_count??0)+' new of '+(progress.expected_artifact_count??0)),scoutText('Safe failure',progress.safe_failure_message||'none'),scoutText('Last status update',progress.last_status_update_at||'unavailable'),refresh);review.prepend(panel);const active=sampleActiveStates.has(progress.operation_state);if(active)root.querySelectorAll('button').forEach(button=>{if(/Generate|Regenerate|Retry Unfinished|Generate Remaining/.test(button.textContent))button.disabled=true});if(active)sampleProgressTimer=setTimeout(()=>renderProducerProject(id),3000)};"
    agency_js += "const validationGateRender=renderProducerProject;renderProducerProject=async id=>{await validationGateRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),sections=[...q('coloring-book-project').querySelectorAll('section')],review=sections[sections.length-1],cards=[...review.querySelectorAll('article.agent-card')];cards.forEach((card,index)=>{const item=(samples.artifacts||[])[index];if(!item)return;const profile=document.createElement('p');profile.textContent=(item.profile_id==='kids-bold-line-art-v2'?'Generated with ':'Profile: ')+(item.profile_id||'kids-bold-line-art-v1');card.insertBefore(profile,card.querySelector('details'));const reasons=item.validation_failed_reasons||[];if(reasons.length){const failed=document.createElement('p');failed.className='dock-badge warning';failed.textContent='Technical validation failed: '+reasons.join('; ');card.insertBefore(failed,card.querySelector('details'))}const approve=[...card.querySelectorAll('button')].find(b=>b.textContent==='Approve');if(approve&&!item.technical_validation?.valid){approve.disabled=true;approve.title='Technical validation must pass before approval.'}});const style=[...review.querySelectorAll('button')].find(b=>b.textContent==='Approve Sample Style Locally'),selected=samples.selected_page_ids||[],approved=new Set((samples.artifacts||[]).filter(x=>x.review_state==='approved'&&x.technical_validation?.valid&&!x.prompt_stale).map(x=>x.page_id));if(style)style.disabled=selected.length!==3||selected.some(id=>!approved.has(id))};"
    agency_js=agency_js.replace("button.disabled=true;q('coloring-book-status').textContent='Regenerating '+v.page_id+' locally with the updated prompt…';const r=await samplePost(route,{confirmed:true,...v}),done=await r.json();","button.disabled=true;q('coloring-book-status').textContent='Regenerating '+v.page_id+' locally…';const pending=samplePost(route,{confirmed:true,...v});setTimeout(()=>renderProducerProject(id),200);const r=await pending,done=await r.json();",1)
    agency_js=agency_js.replace("q('coloring-book-status').textContent='Updated-prompt sample generated locally for review.';await renderProducerProject(id)","q('coloring-book-status').textContent='New page-001 candidate ready for review.';await renderProducerProject(id);document.querySelector('[data-newest-page001=\"true\"]')?.scrollIntoView({behavior:'smooth',block:'center'})",1)
    agency_js += "const candidateHistoryRender=renderProducerProject;renderProducerProject=async id=>{await candidateHistoryRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),root=q('coloring-book-project'),sections=[...root.querySelectorAll('section')],review=sections[sections.length-1],cards=[...review.querySelectorAll('article.agent-card')],pageItems=(samples.artifacts||[]).filter(x=>x.page_id==='page-001').sort((a,b)=>String(a.generated_at||'').localeCompare(String(b.generated_at||''))),newest=pageItems[pageItems.length-1];cards.forEach((card,index)=>{const item=(samples.artifacts||[])[index];if(!item)return;card.dataset.pageId=item.page_id;card.dataset.assetId=item.asset_id;if(newest&&item.asset_id===newest.asset_id){card.dataset.newestPage001='true';card.style.outline='3px solid #3b82f6';card.style.outlineOffset='3px'}});const history=document.createElement('section'),heading=document.createElement('h5'),notice=document.createElement('p');history.className='sample-candidate-history';heading.textContent='page-001 Candidate History';notice.textContent='kids-bold-line-art-v2 is unsuitable for this project unless manually selected. Default regeneration profile: kids-bold-line-art-v1. Marking a reference is local planning metadata, not approval.';history.append(heading,notice);pageItems.forEach((item,index)=>{const row=document.createElement('article'),img=document.createElement('img'),text=document.createElement('p'),button=document.createElement('button');row.className='agent-card';img.src='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(item.asset_id);img.alt='Candidate '+item.asset_id;img.style.width='96px';text.textContent=(item.generated_at||'unknown time')+' · '+(item.profile_id||'kids-bold-line-art-v1')+' · prompt revision '+(item.prompt_revision??0)+' · '+item.review_state+' · technical '+(item.technical_validation?.valid?'valid':'invalid')+' · '+(item.output_designation||'raw')+(index===0?' · first-ever candidate':'');button.type='button';button.textContent=item.reference_candidate?'Reference Candidate':'Mark as Reference Candidate';button.disabled=!!item.reference_candidate;button.onclick=async()=>{const r=await samplePost('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(item.asset_id)+'/reference',{}),v=await r.json();q('coloring-book-status').textContent=r.ok?'Reference candidate saved locally. This is not an approval.':(v.detail||'Reference candidate was not saved.');if(r.ok)renderProducerProject(id)};row.append(img,text,button);history.append(row)});review.append(history);const progress=samples.progress||{};if(progress.operation_type==='regenerate_single_page'&&progress.page_ids?.includes('page-001')){const target=progress.active?root.querySelector('[data-page-id=\"page-001\"]'):root.querySelector('[data-newest-page001=\"true\"]');target?.scrollIntoView({behavior:'smooth',block:'center'})}};"
    agency_js=agency_js.replace("target?.scrollIntoView({behavior:'smooth',block:'center'})","if(target){target.dataset.autoScrolled='true';target.scrollIntoView({behavior:'smooth',block:'center'})}",1)
    agency_js=agency_js.replace("async function regenerateUpdatedPrompt(id,asset,button){","async function regenerateUpdatedPrompt(id,asset,button,profile='kids-bold-line-art-v1'){",1)
    agency_js += "const v3RegenerationRender=renderProducerProject;renderProducerProject=async id=>{await v3RegenerationRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),items=(samples.artifacts||[]).filter(x=>x.page_id==='page-001').sort((a,b)=>String(a.generated_at||'').localeCompare(String(b.generated_at||''))),newest=items[items.length-1],history=q('coloring-book-project').querySelector('.sample-candidate-history');if(history&&newest&&!items.some(x=>x.profile_id==='kids-bold-line-art-v3')){const button=document.createElement('button');button.type='button';button.textContent='Regenerate Once With kids-bold-line-art-v3';button.onclick=()=>regenerateUpdatedPrompt(id,newest.asset_id,button,'kids-bold-line-art-v3');history.append(button)}};"
    agency_js += "const profileControlRender=renderProducerProject;renderProducerProject=async id=>{await profileControlRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),root=q('coloring-book-project'),items=(samples.artifacts||[]).filter(x=>x.page_id==='page-001').sort((a,b)=>String(a.generated_at||'').localeCompare(String(b.generated_at||''))),active=items[items.length-1],cards=[...root.querySelectorAll('article.agent-card[data-page-id]')];root.querySelectorAll('.page-profile-controls').forEach(x=>x.remove());root.querySelectorAll('.sample-candidate-history>button').forEach(x=>x.remove());cards.forEach(card=>{const profileLabels=[...card.querySelectorAll('p')].filter(p=>/^(Profile:|Generated with )/.test(p.textContent));profileLabels.slice(1).forEach(x=>x.remove());const edits=[...card.querySelectorAll('button')].filter(b=>b.textContent==='Edit Prompt');edits.slice(1).forEach(x=>x.remove());[...card.querySelectorAll('button')].filter(b=>/^Regenerate/.test(b.textContent)).forEach(x=>x.remove())});if(!active)return;const card=cards.find(x=>x.dataset.assetId===active.asset_id);if(!card)return;const policy=samples.page_generation_policy||{},controls=document.createElement('div'),label=document.createElement('label'),select=document.createElement('select'),button=document.createElement('button'),attempts=document.createElement('p'),note=document.createElement('p');controls.className='page-profile-controls';label.textContent='Recommended next profile ';for(const profile of ['kids-bold-line-art-v1','kids-bold-line-art-v3','kids-bold-line-art-v4','kids-bold-line-art-v5','kids-bold-line-art-v6']){const option=document.createElement('option');option.value=profile;option.textContent=profile+(profile==='kids-bold-line-art-v6'?' — recommended':'');select.append(option)}select.value='kids-bold-line-art-v6';select.setAttribute('aria-label','Profile for page-001 regeneration');button.type='button';button.textContent='Regenerate page-001 with updated prompt';button.disabled=Boolean(samples.progress?.active)||!active.prompt_details?.override_saved||policy.generation_available===false;button.onclick=()=>regenerateUpdatedPrompt(id,active.asset_id,button,select.value);attempts.textContent='Attempts: '+(policy.attempts_used||0)+' of '+(policy.maximum_attempts_per_page||3);note.setAttribute('role','status');note.textContent=policy.blocked_reason||((policy.new_prompt_revision?'New prompt revision — generation available. ':'')+'V6 uses native SDXL Base locally; no marketplace or publication action will occur.');label.append(select);controls.append(label,attempts,button,note);card.append(controls)};"
    agency_js += "const semanticReviewRender=renderProducerProject;renderProducerProject=async id=>{await semanticReviewRender(id);const response=await fetch('/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples',{cache:'no-store'}),samples=await response.json(),cards=[...q('coloring-book-project').querySelectorAll('article.agent-card[data-asset-id]')];cards.forEach(card=>{card.querySelectorAll('.semantic-review-summary').forEach(x=>x.remove());const item=(samples.artifacts||[]).find(x=>x.asset_id===card.dataset.assetId);if(!item||!['kids-bold-line-art-v5','kids-bold-line-art-v6'].includes(item.profile_id))return;const semantic=item.semantic_review||{},panel=document.createElement('aside'),heading=document.createElement('strong');panel.className='semantic-review-summary';heading.textContent='Semantic review — human confirmation required';const status=value=>value===true?'detected':value===false?'not detected':'not reviewed';panel.append(heading,scoutText('Bear',status(semantic.bear?.detected)+' / expected'),scoutText('Rabbit',status(semantic.rabbit?.detected)+' / expected'),scoutText('Tent',status(semantic.tent?.detected)+' / expected'),scoutText('Unpacking action',status(semantic.unpacking_action?.matched)),scoutText('Certainty','No automated semantic certainty is claimed.'));card.append(panel)})};"
    agency_js += (Path(__file__).resolve().parents[1]/"web"/"producer_workspace.js").read_text(encoding="utf-8")
    agency_js += """let updatedPromptRegenerationBusy=false;
regenerateUpdatedPrompt=async function(id,asset,button,profile='kids-bold-line-art-v1'){
 const status=q('coloring-book-status');
 if(updatedPromptRegenerationBusy||button?.disabled){status.textContent='Regeneration is already active or unavailable. No duplicate request was sent.';return}
 const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/regenerate-with-updated-prompt';
 let previewResponse,preview;
 try{previewResponse=await samplePost(route,{confirmed:false,profile_id:profile});preview=await sampleResponseJson(previewResponse)}
 catch(error){status.textContent='Regeneration preview failed: '+(error.message||String(error))+'. No generation was started.';return}
 if(!previewResponse.ok){status.textContent='Regeneration blocked (HTTP '+previewResponse.status+'): '+(preview.detail||'No generation was started.');return}
 let accepted=false;
 try{accepted=window.confirm(preview.confirmation)}
 catch(error){status.textContent='Regeneration confirmation failed: '+(error.message||String(error))+'. No generation was started.';return}
 if(!accepted){status.textContent='Regeneration canceled. No confirmed request was sent.';return}
 updatedPromptRegenerationBusy=true;if(button)button.disabled=true;
 status.textContent='Regenerating '+preview.page_id+' locally…';
 const keys=['project_id','asset_id','page_id','old_prompt_revision','old_prompt_hash','new_prompt_hash','prompt_revision','positive_prompt_hash','negative_prompt_hash','old_profile','new_profile','profile_id','checkpoint','workflow_hash','postprocessing','validation_thresholds','reference_candidate_id','attempt_number','maximum_attempts_per_page','generation_attempt_identity','request_id','regeneration_identity'];
 const confirmed={confirmed:true};keys.forEach(key=>confirmed[key]=preview[key]);
 let response,result;
 try{response=await samplePost(route,confirmed);result=await sampleResponseJson(response)}
 catch(error){status.textContent='Regeneration result is uncertain because the confirmed request was interrupted: '+(error.message||String(error))+'. Do not retry automatically.';return}
 if(!response.ok){status.textContent='Regeneration failed (HTTP '+response.status+'): '+(result.detail||'No new candidate was created.');updatedPromptRegenerationBusy=false;if(button)button.disabled=false;return}
 const match=(result.artifacts||[]).find(item=>item.page_id===preview.page_id&&item.profile_id===preview.profile_id&&Number(item.prompt_revision)===Number(preview.prompt_revision)&&item.positive_prompt_hash===preview.positive_prompt_hash&&item.negative_prompt_hash===preview.negative_prompt_hash&&item.generation_attempt_identity===preview.generation_attempt_identity);
 if(!match){status.textContent='Regeneration failed safely: the confirmed response contained no new artifact matching this prompt revision and attempt identity.';updatedPromptRegenerationBusy=false;if(button)button.disabled=false;return}
 try{sessionStorage.setItem('coloring-book-producer-tab:'+id,'samples')}catch(error){}
 await renderProducerProject(id);
 const target=document.querySelector('[data-asset-id="'+CSS.escape(match.asset_id)+'"]');if(target){target.dataset.newestPage001='true';target.scrollIntoView({behavior:'smooth',block:'center'})}
 status.textContent='New '+preview.page_id+' candidate ready for review. ComfyUI prompt: '+(match.comfyui_prompt_id||'recorded in operation status')+' · prompt revision '+preview.prompt_revision+'.';
 updatedPromptRegenerationBusy=false;
};"""
    agency_js=agency_js.replace("const optimizedProducerRender=renderProducerProject;","const optimizedProducerRender=promptEditorRender;",1)
    agency_js=agency_js.replace("const cards=[...review.querySelectorAll('article.agent-card[data-page-id=\\\"page-001\\\"]')],items=", "const allCards=[...review.querySelectorAll('article.agent-card')];allCards.forEach((card,index)=>{const item=(samples.artifacts||[])[index];if(item){card.dataset.pageId=item.page_id;card.dataset.assetId=item.asset_id}});const cards=allCards.filter(card=>card.dataset.pageId==='page-001'),items=",1)
    agency_js=agency_js.replace("const items=(samples.artifacts||[]).filter(x=>x.page_id==='page-001').sort((a,b)=>String(a.generated_at||'').localeCompare(String(b.generated_at||''))),current=items[items.length-1]","const items=(samples.artifacts||[]).filter(x=>x.page_id==='page-001').sort((a,b)=>String(a.generated_at||'').localeCompare(String(b.generated_at||''))),current=items[items.length-1]",1)
    agency_js=agency_js.replace("technicalDetails=item=>{const details=", "technicalDetails=item=>{const details=",1)
    agency_js=agency_js.replace("};if(current){const activeCard=", "};if(!current&&samples.project_status==='page_plan_approved'){const generate=document.createElement('button');generate.type='button';generate.textContent='Generate 3 Sample Pages';generate.onclick=()=>generateSamples(id,generate);review.append(generate)}if(current){const activeCard=",1)
    agency_js += "const safeOptimizedProducerRender=renderProducerProject;renderProducerProject=async id=>{try{return await safeOptimizedProducerRender(id)}catch(error){q('coloring-book-status').textContent='Producer workspace optimization failed: '+(error.message||String(error));console.error(error);throw error}};"
    agency_js += "if(activeView==='agency.coloring-book-producer'&&requestedProducerProject)renderProducerProject(requestedProducerProject);"
    agency_js=agency_js.replace("history.replaceState(null,'','/app?view='+encodeURIComponent(view));log('Opened '+titles[view])","const preserved=new URLSearchParams(location.search);preserved.set('view',view);history.replaceState(null,'','/app?'+preserved.toString());log('Opened '+titles[view])",1)
    agency_js=agency_js.replace("section.append(h,state);if(!v.provider_readiness.configured)","section.append(h,state,scoutText('Provider ID',v.provider_readiness.provider_id),scoutText('Workflow profile',v.provider_readiness.profile_id),scoutText('Checkpoint',v.provider_readiness.checkpoint||'missing'),scoutText('Workflow SHA-256',v.provider_readiness.workflow_hash||'missing'),scoutText('Readiness',v.provider_readiness.status),scoutText('Local-only','yes · local GPU generation writes local outputs; no external or marketplace action'));if(!v.provider_readiness.configured)",1)
    agency_js=agency_js.replace("generate.textContent='Generate 3 Sample Pages';generate.onclick=()=>generateSamples(id);section.append(generate)","generate.textContent='Generate 3 Sample Pages';generate.disabled=v.provider_readiness.status!=='ready';generate.title=generate.disabled?'Requires an approved page plan and a ready local workflow profile.':'Uses the approved plan with local GPU effects only.';generate.onclick=()=>generateSamples(id,generate);section.append(generate)",1)
    old_sample_generation="async function generateSamples(id){const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/generate',p=await samplePost(route,{confirmed:false}),v=await p.json();if(!p.ok){q('coloring-book-status').textContent=v.detail;return}if(!v.provider_readiness.configured){q('coloring-book-status').textContent='Local image provider is not configured.';return}if(!window.confirm('Project '+v.project_id+' · plan revision '+v.page_plan_revision+' · '+v.page_plan_hash+' · samples '+v.selected_sample_page_ids.join(', ')+' · workflow '+v.workflow+' · checkpoint '+v.checkpoint+' · local GPU effects only · output '+v.output_directory+' · no PDF, Amazon upload, or publication.'))return;const r=await samplePost(route,{confirmed:true});q('coloring-book-status').textContent=r.ok?'Three local sample pages generated for review.':'Sample generation failed safely.';if(r.ok)renderProducerProject(id)}"
    new_sample_generation="let sampleGenerationBusy=false;async function sampleResponseJson(response){try{return await response.json()}catch(e){return {detail:'The server returned an unreadable response.'}}}async function generateSamples(id,button){const status=q('coloring-book-status');if(sampleGenerationBusy){status.textContent='Sample generation is already in progress. Duplicate submission blocked.';return}const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/generate';let preview;try{const response=await samplePost(route,{confirmed:false});preview=await sampleResponseJson(response);if(!response.ok){status.textContent='Sample preview failed (HTTP '+response.status+'): '+(preview.detail||'No generation was started.');return}}catch(e){status.textContent='Sample preview JavaScript or network failure: '+(e.message||String(e))+'. No generation was started.';return}if(!preview.provider_readiness?.configured){status.textContent='Local image provider is not configured.';return}let accepted=false;try{accepted=window.confirm('Project '+preview.project_id+' · plan revision '+preview.page_plan_revision+' · '+preview.page_plan_hash+' · samples '+preview.selected_page_ids.join(', ')+' · workflow '+preview.workflow_profile+' / '+preview.workflow+' · checkpoint '+preview.checkpoint+' · local GPU effects only · output '+preview.output_directory+' · no PDF, Amazon upload, or publication.')}catch(e){status.textContent='Sample confirmation JavaScript failure: '+(e.message||String(e))+'. No generation was started.';return}if(!accepted){status.textContent='Sample generation canceled. No images were generated.';return}sampleGenerationBusy=true;if(button)button.disabled=true;status.textContent='Generating 3 sample pages locally…';const confirmed={confirmed:true,project_id:preview.project_id,page_plan_revision:preview.page_plan_revision,page_plan_hash:preview.page_plan_hash,selected_page_ids:preview.selected_page_ids,workflow_profile:preview.workflow_profile,workflow:preview.workflow,workflow_hash:preview.workflow_hash,checkpoint:preview.checkpoint,request_id:preview.request_id,generation_identity:preview.generation_identity};let response;try{response=await samplePost(route,confirmed)}catch(e){status.textContent='Sample generation result is uncertain because the confirmed request was interrupted: '+(e.message||String(e))+'. Do not retry automatically; verify ComfyUI and sample status first.';return}const result=await sampleResponseJson(response);if(!response.ok){status.textContent='Sample generation failed (HTTP '+response.status+'): '+(result.detail||'No successful result was returned.');sampleGenerationBusy=false;if(button)button.disabled=false;return}status.textContent=(result.status||'unknown')+': '+(result.message||((result.artifacts||[]).length+' sample artifacts returned.'));try{await renderProducerProject(id)}catch(e){status.textContent='Sample gallery refresh JavaScript failure: '+(e.message||String(e))+'. Generation status: '+(result.status||'unknown')+'.';sampleGenerationBusy=false;return}if(result.status==='review_ready'||result.idempotent){status.textContent=(result.status||'review_ready')+': '+((result.artifacts||[]).length+' sample artifacts ready for review.');sampleGenerationBusy=false;return}if(result.status==='uncertain')status.textContent='uncertain: '+result.message+' Do not retry automatically.';else if(result.status!=='completed')status.textContent=(result.status||'rejected')+': '+(result.message||'ComfyUI submission was rejected or did not complete.');sampleGenerationBusy=false;if(button)button.disabled=false}"
    if old_sample_generation not in agency_js:raise RuntimeError("sample generation handler patch target is unavailable")
    agency_js=agency_js.replace(old_sample_generation,new_sample_generation,1)
    agency_js=agency_js.replace("async function regenerateUpdatedPrompt(id,asset,button,profile='kids-bold-line-art-v1'){if(button.disabled)return;const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/regenerate-with-updated-prompt';try{const p=await samplePost(route,{confirmed:false}),","async function regenerateUpdatedPrompt(id,asset,button,profile='kids-bold-line-art-v1'){if(button.disabled)return;const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/'+encodeURIComponent(asset)+'/regenerate-with-updated-prompt';try{const p=await samplePost(route,{confirmed:false,profile_id:profile}),",1)
    agency_js=agency_js.replace("section.append(h,state,scoutText('Provider ID',v.provider_readiness.provider_id)","section.append(h,state,scoutText('Operation state',v.operation_state||v.status),scoutText('Selected pages',(v.selected_page_ids||[]).join(', ')||'none'),scoutText('Submitted ComfyUI prompts',(v.submitted_prompt_ids||[]).join(', ')||'none'),scoutText('Artifact count',v.artifact_count??(v.artifacts||[]).length),scoutText('Failure',v.safe_failure_message||'none'),scoutText('Provider ID',v.provider_readiness.provider_id)",1)
    agency_js=agency_js.replace("if((v.artifacts||[]).length){const approve=document.createElement('button')","if(v.reconciliation_allowed){const reconcile=document.createElement('button');reconcile.type='button';reconcile.textContent='Reconcile No-Submission and Allow One Retry';reconcile.onclick=async()=>{const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/samples/reconcile',body={confirmed:false},previewResponse=await samplePost(route,body),preview=await sampleResponseJson(previewResponse);if(!previewResponse.ok){q('coloring-book-status').textContent='Reconciliation failed (HTTP '+previewResponse.status+'): '+(preview.detail||'No state changed.');return}if(!window.confirm(preview.confirmation))return;body.confirmed=true;const response=await samplePost(route,body),value=await sampleResponseJson(response);q('coloring-book-status').textContent=response.ok?value.message:('Reconciliation failed (HTTP '+response.status+'): '+(value.detail||'No state changed.'));await renderProducerProject(id)};section.append(reconcile)}if((v.artifacts||[]).length){const approve=document.createElement('button')",1)
    agency_js=agency_js.replace("if(v.reconciliation_allowed){const reconcile",retry_ui+"if(v.reconciliation_allowed){const reconcile",1)
    agency_js=agency_js.replace("reconcile.textContent='Reconcile No-Submission and Allow One Retry'","reconcile.textContent=v.operation_state==='provider_submission_lost_after_restart'?'Review Lost Prompt and Allow One Retry':'Reconcile No-Submission and Allow One Retry'",1)
    agency_js=agency_js.replace("else renderProducerProject(v.projects.find(p=>!p.duplicate_of)?.project_id||v.projects[0].project_id)","else renderProducerProject(initialProducerProject||v.projects.find(p=>!p.duplicate_of)?.project_id||v.projects[0].project_id)")
    agency_js=agency_js.replace("approve.onclick=()=>approvePlan(id)","approve.onclick=()=>approvePlan(id,approve)")
    old_page_plan_approval="async function approvePlan(id){const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/page-plan/approve',body={csrf_token:csrf,confirmed:false},p=await fetch(route,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),v=await p.json();if(!p.ok){q('coloring-book-status').textContent=v.detail;return}if(!window.confirm(v.confirmation))return;body.confirmed=true;const r=await fetch(route,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),done=await r.json();q('coloring-book-status').textContent=r.ok?done.message:done.detail;if(r.ok){loadColoringBookProjects();renderProducerProject(id)}}"
    new_page_plan_approval="let pagePlanApprovalBusy=false;async function approvePlan(id,button){if(pagePlanApprovalBusy)return;const route='/app/agency/coloring-book-producer/projects/'+encodeURIComponent(id)+'/page-plan/approve',body={csrf_token:csrf,confirmed:false},status=q('coloring-book-status');status.textContent='Preparing page-plan approval preview…';let preview;try{preview=await fetch(route,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})}catch(e){status.textContent='Approval preview network error: '+(e.message||'request failed')+'. Nothing was approved.';return}let value={};try{value=await preview.json()}catch(e){value={detail:'Unreadable server response.'}}if(!preview.ok){status.textContent='Approval preview failed (HTTP '+preview.status+'): '+(value.detail||'Nothing was approved.');return}if(!window.confirm(value.confirmation)){status.textContent='Page-plan approval canceled. Nothing changed.';return}pagePlanApprovalBusy=true;if(button)button.disabled=true;status.textContent='Approving page plan…';body.confirmed=true;let response;try{response=await fetch(route,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)})}catch(e){status.textContent='Approval network error: '+(e.message||'request failed')+'. Verify state before retrying.';pagePlanApprovalBusy=false;if(button)button.disabled=false;return}let done={};try{done=await response.json()}catch(e){done={detail:'Unreadable server response.'}}if(!response.ok){status.textContent='Approval failed (HTTP '+response.status+'): '+(done.detail||'The plan was not approved.');pagePlanApprovalBusy=false;if(button)button.disabled=false;return}status.textContent=done.message||'Page plan approved locally. No images or external actions occurred.';await loadColoringBookProjects();await renderProducerProject(id);pagePlanApprovalBusy=false}"
    if old_page_plan_approval not in agency_js:raise RuntimeError("page-plan approval handler patch target is unavailable")
    agency_js=agency_js.replace(old_page_plan_approval,new_page_plan_approval,1)
...
@app.get("/app/agency/coloring-book-producer/projects/{project_id}/samples")
def coloring_book_sample_status_route(project_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer()
    try:return ColoringBookProducer().sample_status(project_id)
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.get("/app/agency/coloring-book-producer/projects/{project_id}/samples/{asset_id}")
def coloring_book_sample_asset_route(project_id:str,asset_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer()
    try:return FileResponse(ColoringBookProducer().sample_asset(project_id,asset_id),media_type="image/png",headers={"Cache-Control":"private, no-store"})
    except (ValueError,OSError,KeyError,json.JSONDecodeError):raise HTTPException(status_code=404,detail="Sample asset unavailable")

async def _sample_mutation(request,callback):
    _validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    try:return callback(values)
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/generate")
async def coloring_book_sample_generate_route(project_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    identity={key:values.get(key) for key in ("project_id","page_plan_revision","page_plan_hash","selected_page_ids","workflow_profile","workflow","workflow_hash","checkpoint","request_id","generation_identity")}
    try:return ColoringBookProducer().generate_samples(project_id,confirmed=values.get("confirmed") is True,candidates_per_page=int(values.get("candidates_per_page") or 1),generation_identity=identity if values.get("confirmed") is True else None)
    except SampleGenerationConflict as exc:raise HTTPException(status_code=409,detail=str(exc)[:300]) from exc
    except SampleGenerationFailure as exc:raise HTTPException(status_code=502,detail=str(exc)[:300]) from exc
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/reconcile")
async def coloring_book_sample_reconcile_route(project_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    try:return ColoringBookProducer().reconcile_sample_generation(project_id,confirmed=values.get("confirmed") is True)
    except SampleGenerationConflict as exc:raise HTTPException(status_code=409,detail=str(exc)[:300]) from exc
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/retry")
async def coloring_book_sample_retry_route(project_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    keys=("project_id","retry_page_ids","original_lost_prompt_id","page_plan_revision","page_plan_hash","workflow_profile","workflow_hash","checkpoint","request_id","generation_identity","comfyui_instance_identity","retry_attempt","retry_identity")
    try:return ColoringBookProducer().retry_unfinished_samples(project_id,confirmed=values.get("confirmed") is True,retry_identity={key:values.get(key) for key in keys} if values.get("confirmed") is True else None)
    except SampleGenerationConflict as exc:raise HTTPException(status_code=409,detail=str(exc)[:300]) from exc
    except SampleGenerationFailure as exc:raise HTTPException(status_code=502,detail=str(exc)[:300]) from exc
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/{asset_id}/review")
async def coloring_book_sample_review_route(project_id:str,asset_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();return await _sample_mutation(request,lambda v:ColoringBookProducer().review_sample(project_id,asset_id,str(v.get("action") or "")))

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/{asset_id}/regenerate")
async def coloring_book_sample_regenerate_route(project_id:str,asset_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();return await _sample_mutation(request,lambda v:ColoringBookProducer().regenerate_sample(project_id,asset_id,confirmed=v.get("confirmed") is True))

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/pages/{page_id}/prompt")
async def coloring_book_sample_prompt_route(project_id:str,page_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();return await _sample_mutation(request,lambda v:ColoringBookProducer().edit_sample_prompt(project_id,page_id,v.get("positive_prompt"),v.get("negative_prompt")))

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/{asset_id}/regenerate-with-updated-prompt")
async def coloring_book_sample_prompt_regenerate_route(project_id:str,asset_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    keys=("project_id","asset_id","page_id","old_prompt_revision","old_prompt_hash","new_prompt_hash","prompt_revision","positive_prompt_hash","negative_prompt_hash","old_profile","new_profile","profile_id","checkpoint","workflow_hash","postprocessing","validation_thresholds","reference_candidate_id","attempt_number","maximum_attempts_per_page","generation_attempt_identity","request_id","regeneration_identity")
    try:return ColoringBookProducer().regenerate_with_updated_prompt(project_id,asset_id,confirmed=values.get("confirmed") is True,regeneration_identity={key:values.get(key) for key in keys} if values.get("confirmed") is True else None,profile_id=str(values.get("profile_id") or values.get("new_profile") or "kids-bold-line-art-v1"))
    except SampleGenerationConflict as exc:raise HTTPException(status_code=409,detail=str(exc)[:300]) from exc
    except SampleGenerationFailure as exc:raise HTTPException(status_code=502,detail=str(exc)[:300]) from exc
    except (ValueError,OSError,KeyError,json.JSONDecodeError) as exc:raise HTTPException(status_code=422,detail=str(exc)[:300]) from exc

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/{asset_id}/reference")
async def coloring_book_sample_reference_route(project_id:str,asset_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();return await _sample_mutation(request,lambda v:ColoringBookProducer().mark_reference_candidate(project_id,asset_id))

@app.post("/app/agency/coloring-book-producer/projects/{project_id}/samples/approve-style")
async def coloring_book_sample_approve_route(project_id:str,request:Request):
    _require_local(request);_require_coloring_book_producer();return await _sample_mutation(request,lambda v:ColoringBookProducer().approve_sample_style(project_id,confirmed=v.get("confirmed") is True))
