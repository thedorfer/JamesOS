...
def application_shell_route(request:Request):
    ...
    for profile in profiles:
        ...
        profile_ui[str(profile.get("profile_id") or "")]= {
            ...
            "form_defaults":{"listing_title":config.get("listing_title") or ""},
        }
    ...
    page=("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>JamesOS</title><style>"
        ...
        "let healthState='';async function pollHealth(){try{const response=await fetch('/app/health',{cache:'no-store'});if(!response.ok)throw new Error();const value=await response.json();if(value.state!==healthState){healthState=value.state;const dot=q('health-dot');dot.className='health-dot '+value.state;dot.setAttribute('aria-label','System health: '+value.label);q('health-label').textContent=value.label;q('health-systems').replaceChildren(...value.systems.map(system=>{const item=document.createElement('li');item.textContent=system.label+': '+system.status+' — '+system.message;return item}))}}catch(e){if(healthState!=='red'){healthState='red';q('health-dot').className='health-dot red';q('health-dot').setAttribute('aria-label','System health: API/server unavailable');q('health-label').textContent='A required local subsystem is unavailable'}}setTimeout(pollHealth,15000)}q('health-dot').onclick=()=>{const panel=q('health-detail'),open=panel.hidden;panel.hidden=!open;q('health-dot').setAttribute('aria-expanded',String(open))};pollHealth();"
        "async function loadAccessStatus(){try{const response=await fetch('/app/access-status',{cache:'no-store'});if(!response.ok)throw new Error();const value=await response.json();q('access-mode').textContent=value.access_mode;q('access-host').textContent=value.trusted_hostname;q('access-https').textContent=value.https?'HTTPS':'plain HTTP';q('access-connection').textContent=value.connection_type;q('access-scope').textContent=value.access_scope;q('access-warning').hidden=!value.warning;q('access-warning').textContent=value.warning||''}catch(e){q('access-mode').textContent='unavailable';q('access-warning').hidden=false;q('access-warning').textContent='Access status is unavailable.'}}loadAccessStatus();"
        "});</script></body></html>")
