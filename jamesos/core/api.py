...
def commerce_new_route(request:Request):
    ...
    page=("<!doctype html><meta charset='utf-8'><meta name='viewport' content='width=device-width'><title>New commerce product</title><style>body{font:16px system-ui;max-width:1200px;margin:auto;padding:2rem}.layout{display:grid;grid-template-columns:minmax(300px,.8fr) minmax(420px,1.2fr);gap:1.5rem}.copilot,.product{border:2px solid #ccd;padding:1rem;border-radius:12px}.cards{display:grid;gap:1rem}.card{border:2px solid #ccd;padding:1rem;border-radius:10px}textarea,input[type=text]{width:100%;box-sizing:border-box;margin:.4rem 0 1rem;padding:.6rem}.actions{display:flex;flex-wrap:wrap;gap:.4rem}.drawer-toggle{display:none}@media(max-width:760px){body{padding:1rem}.layout{display:block}.drawer-toggle{display:block}.copilot[hidden]{display:none}.product{margin-top:1rem}}</style>"
        f"<button type='button' class='drawer-toggle' id='copilot-toggle'>Commerce Copilot</button><main class='layout'><aside class='copilot' id='copilot'><h1>Commerce Copilot</h1><p>Local suggestions only. It cannot contact shops or submit this form.</p><textarea id='copilot-message' maxlength='2000' placeholder='Ask for a phrase, brief, title, colors, tags, review, or validation help'></textarea><button type='button' id='copilot-send'>Ask Copilot</button><p id='copilot-response'></p><p id='copilot-colors'></p><p id='copilot-artwork'></p><p id='copilot-tags'></p><div class='actions' id='copilot-actions'></div></aside><section class='product'><h1>New Commerce Product</h1><form id='commerce-form' method='post' action='/commerce/new'><input type='hidden' name='csrf_token' value='{html_escape(_COMMERCE_CREATE_CSRF,quote=True)}'><div class='cards'>{''.join(cards)}</div>"
        ...
        f"<input type='hidden' name='destination_confirmed' value='true'><button id='generate'>Generate unpublished draft</button><p id='destination'></p></form></section></main><script>const csrf='{html_escape(_COMMERCE_CREATE_CSRF,quote=True)}',r=[...document.querySelectorAll('input[name=commerce_profile_id]')],b=document.getElementById('generate'),d=document.getElementById('destination'),fields=['exact_phrase','product_brief','listing_title','special_instructions'];let session=crypto.randomUUID(),suggestions={{}};function u(){{const x=r.find(x=>x.checked);if(x){{b.textContent='Generate unpublished draft in '+x.dataset.destination;d.textContent='Destination: '+x.dataset.destination;session=crypto.randomUUID()}}}}r.forEach(x=>x.addEventListener('change',u));u();document.getElementById('copilot-toggle').onclick=()=>{{const c=document.getElementById('copilot');c.hidden=!c.hidden}};function apply(k){{if(suggestions[k]!==undefined)document.getElementById(k).value=suggestions[k]}}function buttons(){{const a=document.getElementById('copilot-actions');a.replaceChildren();for(const [label,key] of [['Apply exact phrase','exact_phrase'],['Apply product brief','product_brief'],['Apply listing title','listing_title'],['Apply special instructions','special_instructions'],['Apply all','all']]){{let q=document.createElement('button');q.type='button';q.textContent=label;q.onclick=()=>key==='all'?fields.forEach(apply):apply(key);a.appendChild(q)}}}}document.getElementById('copilot-send').onclick=async()=>{{const x=r.find(x=>x.checked),form=Object.fromEntries(fields.map(k=>[k,document.getElementById(k).value]));let response=await fetch('/commerce/copilot/message',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{csrf_token:csrf,session_id:session,profile_id:x.value,message:document.getElementById('copilot-message').value,form}})}}),data=await response.json();if(!response.ok){{document.getElementById('copilot-response').textContent=data.detail||'Copilot request failed';return}}suggestions=data.suggestions;document.getElementById('copilot-response').textContent=data.response;document.getElementById('copilot-colors').textContent='Garment colors: '+data.garment_colors.join(', ');document.getElementById('copilot-artwork').textContent='Artwork palette: '+data.artwork_palette.join(', ');document.getElementById('copilot-tags').textContent=(data.tags_valid?'13 validated tags: ':'Tags need review: ')+data.tags.join(', ');buttons()}}</script>")
...
async def commerce_resume_existing_draft_route(job_id:str,request:Request):
    ...
    values={key:items[-1] for key,items in parse_qs((await request.body()).decode(),keep_blank_values=True).items()}
    if not hmac.compare_digest(values.get("csrf_token",""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    result=CommerceCreationService().resume_existing_draft(job_id)
