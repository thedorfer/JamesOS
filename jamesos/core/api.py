...
async def commerce_resume_existing_draft_route(job_id:str,request:Request):
    ...
    if state.get("stage")=="awaiting_final_approval":result=service.open_product_review(job_id)
    else:
        status=service.safe_status(job_id);result=service.open_product_review(job_id) if status.get("open_product_review_allowed") else service.resume_existing_draft(job_id)
