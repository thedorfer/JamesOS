...
def application_shell_route(request:Request):
    ...
    page=page.replace("if(!allowedViews.includes(view))return;activeView=view", "if(typeof view!=='string'||!allowedViews.includes(view))return;const title=({'dashboard':'Home','agency.home':'The Agency','agency.agent':'Agent details','admin.home':'Admin','commerce.new':'Product Studio','commerce.loading':'Generating product','commerce.review':'Product review','commerce.diagnostics':'Commerce diagnostics','commerce.published':'Published product','jobs.list':'Jobs','jobs.detail':'Job detail','profiles':'Profiles','settings':'Settings','diagnostics':'Diagnostics'})[view];if(!title)return;const enteringProduct=view==='commerce.new'&&activeView!=='commerce.new';if(enteringProduct){q('listing_title').value='';fieldMeta.listing_title={dirty:false,autoValue:''}}activeView=view")
    ...
    if active=="commerce.review" and requested_job:
        try:
            ...
            review_html=(f"<section id='commerce-review'><h3>Product review</h3><p class='safeguard'><strong>Ready for review</strong><br>UNPUBLISHED DRAFT · NO ORDER CREATED</p>{preview}"
                ...
                f"<h4>{html_escape(str(review.get('listing_title') or 'Untitled listing'))}</h4><p>{html_escape(str(review.get('description') or ''))}</p><p><strong>Etsy tags ({len(review['tags'])}):</strong></p><ol id='review-tags'>{tags}</ol>"
                f"<p><strong>Artwork palette:</strong> {html_escape(', '.join(map(str,review.get('artwork_palette') or [])))}<br><strong>Garment colors:</strong> {html_escape(', '.join(map(str,review.get('garment_colors') or [])))}</p>"
                f"<p><strong>Provider contacted:</strong> {'yes' if review.get('provider_contacted') else 'no'}<br><strong>Workflow timeline:</strong> {timeline}<br><strong>Publication:</strong> no<br><strong>Order:</strong> no</p>"
                f"<div class='actions'><button type='button' id='open-draft-details'>Open Printify draft details</button><a class='button' href='{html_escape(str(review.get('artwork_url') or '#'),quote=True)}'>Review artwork</a><button type='button' data-view='commerce.new'>Revise listing metadata</button><button type='button' data-view='commerce.new'>Regenerate local artwork</button><button type='button' data-view='commerce.new'>Return to Product Studio</button></div></section>")
