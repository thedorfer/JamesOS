...
def application_shell_route(request:Request):
    _require_local(request);profiles=list_commerce_profiles(enabled_only=True);profile_ids={str(item.get("profile_id") or "") for item in profiles};active=str(request.query_params.get("view") or "dashboard");requested_job=str(request.query_params.get("job_id") or "");agency_snapshot=ShellAgencyRegistry().snapshot();private_policy=PrivateChatPolicy().status()
    ...
    for agent in agency_snapshot["agents"]:
        aid=html_escape(agent["agent_id"],quote=True);workspace=agent["workspace"];open_label="Open Product Studio" if workspace=="commerce.new" else "Open Admin" if workspace=="admin.home" else "Open Home"
    ...
    for item in agency_snapshot["catalog"]:
        ...
        marketplace.append(f"<article class='agent-card'><h4>{html_escape(item['name'])}</h4><p>{html_escape(item['publisher'])} · {html_escape(item['version'])} · {'Verified' if item['verified'] else 'Unverified'}</p><p>{html_escape(item['description'])}</p><details data-layout-locked='true'><summary>Permission review</summary><p>{html_escape(permissions)}</p><p>Confirmation: {html_escape(', '.join(item['confirmation_requirements']))}</p></details><button type='button' disabled>Planned</button></article>")
    ...
    agency_html=("<section id='agency-view' hidden><h3>The Agency</h3><nav class='agency-tabs' aria-label='Agency sections'>"+"".join(f"<button type='button' data-agency-tab='{key}'>{label}</button>" for key,label in (("my-agents","My Agents"),("marketplace","Marketplace"),("runs","Runs"),("approvals","Approvals"),("updates","Updates")))+"</nav>"
        ...
        "<section class='agency-section' id='agency-agent-detail' data-layout-locked='true' hidden><h4>Agent details</h4><div id='agency-agent-detail-content'></div><button type='button' data-agency-tab='my-agents'>Back to My Agents</button></section></section>")
    ...
    page=page.replace("if(!allowedViews.includes(view))return;activeView=view", "if(typeof view!=='string'||!allowedViews.includes(view))return;const title=({'dashboard':'Home','agency.home':'The Agency','agency.agent':'Agent details','admin.home':'Admin','commerce.new':'Product Studio','commerce.loading':'Generating product','commerce.artwork-review':'Artwork review','commerce.review':'Product review','commerce.diagnostics':'Commerce diagnostics','commerce.published':'Published product','jobs.list':'Jobs','jobs.detail':'Job detail','profiles':'Profiles','settings':'Settings','diagnostics':'Diagnostics'})[view];if(!title)return;const enteringProduct=view==='commerce.new'&&activeView!=='commerce.new';if(enteringProduct){q('listing_title').value='';fieldMeta.listing_title={dirty:false,autoValue:''}}activeView=view")
    ...
    page=page.replace("history.replaceState(null,'','/app?view='+encodeURIComponent(view));if(view==='admin.home'", "const nextUrl='/app?view='+encodeURIComponent(view)+(selectedJob?'&job_id='+encodeURIComponent(selectedJob):'');history.replaceState(null,'',nextUrl);if(view==='admin.home'")
    ...
    artwork_review_html="<section id='commerce-artwork-review' hidden><h3>Artwork review</h3><p>Generate local candidates before selecting artwork.</p></section>"
    if active=="commerce.artwork-review" and requested_job:
        try:
            artwork=CommerceCreationService().artwork_review_snapshot(requested_job);cards=[]
            previews={item["candidate_id"]:[row for row in artwork["local_previews"] if row["candidate_id"]==item["candidate_id"]] for item in artwork["candidates"]}
            for item in artwork["candidates"]:
                garments="".join(f"<figure><img src='{html_escape(row['url'],quote=True)}' alt='{html_escape(row['garment_color'])} local garment preview'><figcaption>Local preview — not a Printify mockup · {html_escape(row['garment_color'])}</figcaption></figure>" for row in previews[item["candidate_id"]])
                cards.append(f"<article class='agent-card artwork-candidate' data-artwork-candidate='{html_escape(item['candidate_id'],quote=True)}' data-candidate-digest='{html_escape(item['digest'],quote=True)}' data-selected='{'true' if item['selected'] else 'false'}'><img class='candidate-artwork' src='{html_escape(item['url'],quote=True)}' alt='{html_escape(item['candidate_name'])} artwork preview'><h4>{html_escape(item['candidate_name'])}</h4><p>Layout: {html_escape(str(item['layout_style']))}<br>Dimensions: {item['dimensions'][0]} × {item['dimensions'][1]}<br>Transparency: {'pass' if item['transparency'] else 'fail'}<br>Artwork palette: {html_escape(', '.join(map(str,item['palette'])))}<br>Occupied bounds: {html_escape(str(item['occupied_bounds']))}<br>Safe margin: {html_escape(item['safe_margin_result'])}<br>Thumbnail readability: {html_escape(item['thumbnail_readability_result'])}<br>Novelty: {html_escape(item['novelty_result'])}<br><strong class='selection-state'>{'Selected' if item['selected'] else 'Not selected'}</strong></p><button type='button' data-select-artwork>Select this artwork</button><div class='local-preview-grid'>{garments}</div></article>")
            artwork_review_html=(f"<section id='commerce-artwork-review' data-job-id='{html_escape(requested_job,quote=True)}' data-selection-revision='{artwork['selection_revision']}'><h3>Artwork review</h3><p><strong>Candidate set {artwork['candidate_set_revision']}</strong> · Provider contacted: no · Printify image: none · Printify draft: none · Publication: no · Etsy listing: no · Order: no</p><div role='group' aria-label='Artwork preview background'><button type='button' data-artwork-background='black'>Black</button><button type='button' data-artwork-background='navy'>Navy</button><button type='button' data-artwork-background='dark-heather'>Dark heather</button><button type='button' data-artwork-background='white'>White</button><button type='button' data-artwork-background='transparency'>Transparency checkerboard</button></div><div class='agent-card-grid' id='artwork-candidate-gallery'>{''.join(cards)}</div><div class='actions'><button type='button' id='regenerate-artwork-candidates'>Regenerate local candidates</button><button type='button' data-view='commerce.new'>Edit Product Studio brief</button><button type='button' id='cancel-artwork-job'>Cancel job</button><button type='button' id='continue-selected-artwork' {'disabled' if not artwork['selected_candidate_id'] else ''}>Continue with selected artwork</button></div><p id='artwork-review-status' aria-live='polite'></p></section>")
        except Exception:artwork_review_html="<section id='commerce-artwork-review'><h3>Artwork review unavailable</h3><p>The job is not currently waiting for provider-free artwork selection.</p></section>"
    ...
    page=page.replace("<section id='generic-view' hidden><h3 id='generic-title'></h3><p id='generic-copy'></p></section>",dashboard+artwork_review_html+review_html+"<section id='generic-view' hidden><h3 id='generic-title'></h3><p id='generic-copy'></p></section>")
    ...
    page=page.replace("if(status.failed){navigate('diagnostics');q('generic-copy').textContent=status.failure_message_safe||'Product preparation did not complete.';return}","if(status.failed){navigate('commerce.diagnostics');q('product-failure').hidden=false;q('generic-copy').textContent=status.failure_message_safe||'Product preparation did not complete.';q('failure-stage').textContent=(status.failed_stage?'Failed stage: '+status.failed_stage+' · ':'')+'Last completed stage: '+(status.last_completed_stage||'none');q('failure-draft').textContent='Printify draft exists: '+(status.printify_draft_exists?'yes':'no');q('failure-published').textContent='Anything published: '+(status.publication_status==='published'?'yes':'no');q('failure-order').textContent='Order exists: '+(status.order_status==='created'?'yes':'no');const d=status.artwork_diagnostics||{},rows=(d.candidates||[]).flatMap(c=>c.rejections||[]).map(x=>x.code+': '+x.explanation);q('artwork-diagnostics').textContent='Image generation: '+(d.image_generation_readiness||'unknown')+' · Generated: '+(d.generated_candidate_count||0)+' · Technical: '+(d.technically_eligible_count||0)+' · Accepted: '+(d.accepted_candidate_count||0)+' · Rejected: '+(d.rejected_candidate_count||0)+(rows.length?' · '+rows.join(' · '):d.zero_candidate_rejection?' · '+d.zero_candidate_rejection.code+': '+d.zero_candidate_rejection.explanation:'');q('retry-local-artwork').disabled=!status.retry_allowed;return}")
    ...
    page=page.replace("q('dashboard-view').hidden=view!=='dashboard';if(q('product-failure')", "q('dashboard-view').hidden=view!=='dashboard';if(q('commerce-review'))q('commerce-review').hidden=view!=='commerce.review';if(q('commerce-artwork-review'))q('commerce-artwork-review').hidden=view!=='commerce.artwork-review';if(view==='commerce.review'||view==='commerce.artwork-review')q('generic-view').hidden=true;if(q('product-failure')")
    ...
    artwork_js="""const artworkReview=q('commerce-artwork-review');if(artworkReview&&artworkReview.dataset.jobId){const job=artworkReview.dataset.jobId,status=q('artwork-review-status'),continueButton=q('continue-selected-artwork');document.querySelectorAll('[data-artwork-background]').forEach(button=>button.onclick=()=>{const backgrounds={black:'#111',navy:'#18233a','dark-heather':'#3e3f42',white:'#fff',transparency:'repeating-conic-gradient(#bbb 0 25%,#eee 0 50%) 0/24px 24px'};document.querySelectorAll('.candidate-artwork').forEach(image=>image.style.background=backgrounds[button.dataset.artworkBackground])});document.querySelectorAll('[data-select-artwork]').forEach(button=>button.onclick=async()=>{const card=button.closest('[data-artwork-candidate]'),body={csrf_token:csrf,candidate_id:card.dataset.artworkCandidate,candidate_digest:card.dataset.candidateDigest,selection_revision:Number(artworkReview.dataset.selectionRevision)},response=await fetch('/commerce/jobs/'+encodeURIComponent(job)+'/select-artwork',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}),value=await response.json();if(!response.ok){status.textContent='Selection was not saved; refresh the review.';return}artworkReview.dataset.selectionRevision=value.selection_revision;document.querySelectorAll('[data-artwork-candidate]').forEach(row=>{const selected=row===card;row.dataset.selected=String(selected);row.querySelector('.selection-state').textContent=selected?'Selected':'Not selected'});continueButton.disabled=false;status.textContent='Selected locally. No provider has been contacted.'});q('regenerate-artwork-candidates').onclick=async()=>{const response=await fetch('/commerce/jobs/'+encodeURIComponent(job)+'/regenerate-artwork-candidates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({csrf_token:csrf})});if(response.ok)location.reload();else status.textContent='Local regeneration did not complete.'};q('cancel-artwork-job').onclick=async()=>{const response=await fetch('/commerce/jobs/'+encodeURIComponent(job)+'/cancel-artwork-review',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({csrf_token:csrf})});if(response.ok)location.href='/app?view=commerce.new'};continueButton.onclick=async()=>{continueButton.disabled=true;status.textContent='Running provider-free preflight, then creating one unpublished draft…';const response=await fetch('/commerce/jobs/'+encodeURIComponent(job)+'/continue-selected-artwork',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({csrf_token:csrf,selection_revision:Number(artworkReview.dataset.selectionRevision)})}),value=await response.json();if(response.ok)location.href=value.review_url||('/app?view=commerce.review&job_id='+encodeURIComponent(job));else{continueButton.disabled=false;status.textContent='The selected artwork could not be continued safely.'}}}"""
    extra=artwork_js+profile_js+extra
    ...
    page=page.replace("if(status.ready_for_review){", "if(status.artwork_review_required){location.href=status.artwork_review_url;return}if(status.ready_for_review){")
    page=page.replace("</style>",".artwork-candidate[data-selected=true]{border:2px solid var(--ready)}.candidate-artwork{width:100%;max-height:420px;object-fit:contain;background:#111}.local-preview-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:.5rem}.local-preview-grid img{width:100%}.local-preview-grid figure{margin:0}</style>",1)
...
def _artwork_review_asset(job_id:str,asset_id:str,kind:str)->Path:
    service=CommerceCreationService();snapshot=service.artwork_review_snapshot(job_id);state=service.orchestrator.load(job_id);root=service.orchestrator._path(job_id).parent.resolve();item=None
    if kind=="candidate":item=next((row for row in (state.get("evidence") or {}).get("candidates") or [] if row.get("candidate_id")==asset_id and any(view.get("candidate_id")==asset_id for view in snapshot["candidates"])),None)
    else:item=next((row for row in (state.get("evidence") or {}).get("local_garment_previews") or [] if row.get("asset_id")==asset_id and any(view.get("asset_id")==asset_id for view in snapshot["local_previews"])),None)
    if not item:raise HTTPException(status_code=404,detail="Artwork review asset unavailable")
    try:path=Path(str(item.get("png_path") if kind=="candidate" else item.get("local_path"))).resolve(strict=True)
    except OSError:raise HTTPException(status_code=404,detail="Artwork review asset unavailable")
    digest=str(item.get("png_sha256") if kind=="candidate" else item.get("sha256") or "")
    if root not in path.parents or product_orchestrator._file_sha(path)!=digest:raise HTTPException(status_code=403,detail="Artwork review asset forbidden")
    try:
        with Image.open(path) as image:image.verify();valid=image.format=="PNG"
    except (OSError,ValueError):valid=False
    if not valid:raise HTTPException(status_code=404,detail="Artwork review asset unavailable")
    return path


@app.get("/commerce/jobs/{job_id}/artwork-candidates/{candidate_id}")
def commerce_artwork_candidate_route(job_id:str,candidate_id:str,request:Request):
    _require_read_only_asset(request);return FileResponse(_artwork_review_asset(job_id,candidate_id,"candidate"),media_type="image/png",headers={"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"})


@app.get("/commerce/jobs/{job_id}/local-previews/{asset_id}")
def commerce_local_preview_route(job_id:str,asset_id:str,request:Request):
    _require_read_only_asset(request);return FileResponse(_artwork_review_asset(job_id,asset_id,"preview"),media_type="image/png",headers={"Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff"})


@app.post("/commerce/jobs/{job_id}/select-artwork")
async def commerce_select_artwork_route(job_id:str,request:Request):
    _require_local(request);_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    try:return CommerceCreationService().select_artwork(job_id,str(values.get("candidate_id") or ""),str(values.get("candidate_digest") or ""),expected_revision=int(values.get("selection_revision",-1)))
    except (TypeError,ValueError) as exc:raise HTTPException(status_code=422,detail="Invalid artwork selection revision") from exc
    except JamesOSError as exc:raise HTTPException(status_code=409,detail=exc.user_message) from exc


@app.post("/commerce/jobs/{job_id}/continue-selected-artwork")
async def commerce_continue_selected_artwork_route(job_id:str,request:Request):
    _require_local(request);_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    try:return CommerceCreationService().continue_with_selected_artwork(job_id,expected_revision=int(values.get("selection_revision",-1)))
    except (TypeError,ValueError) as exc:raise HTTPException(status_code=422,detail="Invalid artwork selection revision") from exc
    except JamesOSError as exc:handle_error(exc,operation="commerce_creation.continue_artwork",context={"job_id":job_id});raise HTTPException(status_code=409,detail=exc.user_message) from exc


@app.post("/commerce/jobs/{job_id}/regenerate-artwork-candidates")
async def commerce_regenerate_artwork_candidates_route(job_id:str,request:Request):
    _require_local(request);_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    try:return CommerceCreationService().regenerate_artwork_review(job_id)
    except JamesOSError as exc:raise HTTPException(status_code=409,detail=exc.user_message) from exc


@app.post("/commerce/jobs/{job_id}/cancel-artwork-review")
async def commerce_cancel_artwork_review_route(job_id:str,request:Request):
    _require_local(request);_validate_commerce_origin(request);values=await request.json()
    if not hmac.compare_digest(str(values.get("csrf_token") or ""),_COMMERCE_CREATE_CSRF):raise HTTPException(status_code=403,detail="Invalid same-origin token")
    service=CommerceCreationService();state=service.orchestrator.load(job_id)
    if state.get("stage")!="artwork_review" or (state.get("evidence") or {}).get("upload") or (state.get("evidence") or {}).get("draft"):raise HTTPException(status_code=409,detail="Job cannot be canceled at this stage")
    state["stage"]="canceled";product_orchestrator._atomic_json(service.orchestrator._path(job_id),state);return {"job_id":job_id,"stage":"canceled","provider_contacted":False}
