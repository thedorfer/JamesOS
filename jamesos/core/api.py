...
def application_shell_route(request:Request):
    ...
    chat_diag=chat_diagnostics();chat_panel=("<article class='layout-panel' data-panel-id='chat_diagnostics'><header class='panel-title'>Chat diagnostics <span></span></header><div class='panel-content'>"
        f"<p>Ollama: {html_escape('reachable' if chat_diag.get('reachable') else 'unreachable' if chat_diag.get('reachable') is False else 'not tested')}</p><p>Configured model: {html_escape(str(chat_diag.get('model') or 'configured locally'))}</p><p>Endpoint mode: {html_escape(str(chat_diag.get('endpoint_mode') or 'generate'))}</p><p>Last request status: {html_escape(str(chat_diag.get('http_status') or 'none'))}</p><p>Response shape: {html_escape(str(chat_diag.get('shape') or 'not requested'))}</p><p>Returned text length: {int(chat_diag.get('text_length') or 0)}</p><p>Last sanitized failure stage: {html_escape(str(chat_diag.get('failure_stage') or 'none'))}</p></div></article>")
