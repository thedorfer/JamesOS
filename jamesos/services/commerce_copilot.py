...
from jamesos.services.ollama_service import ask_ollama
...
class CommerceCopilotService:
    def __init__(self,*,model:Callable[[str],str]=ask_ollama,root:Path=ROOT):self.model=model;self.root=root

    def message(self,*,session_id:str,profile:dict[str,Any],message:str,form:dict[str,Any])->dict[str,Any]:
        if not SESSION_RE.fullmatch(session_id):raise ValidationError("VALIDATION_FAILED",diagnostic_message="Copilot session ID is invalid.",operation="commerce_copilot",stage="input")
        message=" ".join(str(message).split())
        if not message or len(message)>2000:raise ValidationError("VALIDATION_FAILED",diagnostic_message="Copilot message is empty or too long.",operation="commerce_copilot",stage="input")
        ...
        if history.get("profile_id")!=context["profile_id"]:raise ValidationError("VALIDATION_FAILED",diagnostic_message="Copilot session is bound to a different commerce profile.",operation="commerce_copilot",stage="profile")
        prompt=("You are Commerce Copilot. Return JSON only with keys: response, exact_phrase, product_brief, garment_colors, artwork_palette, listing_title, special_instructions, tags, concerns. "
        ...
        raw=self.model(prompt);raw=str(raw).strip()
        if raw.startswith("```"):raw=re.sub(r"^```(?:json)?\s*|\s*```$","",raw,flags=re.I)
        try:value=json.loads(raw)
        except (TypeError,json.JSONDecodeError) as exc:raise ValidationError("VALIDATION_FAILED",diagnostic_message="Commerce Copilot returned an invalid structured response.",operation="commerce_copilot",stage="model") from exc
        ...
        try:tag_result=finalize_listing_tags(value.get("tags") or [],profile,title)
        ...
        result={"response":" ".join(str(value.get("response") or "").split())[:4000],"suggestions":suggestions,
            "garment_colors":garment,"artwork_palette":artwork,"tags":tag_result["final_listing_tags"],"tags_valid":len(tag_result["final_listing_tags"])==13,
            "tag_diagnostics":tag_result,"concerns":[" ".join(str(item).split())[:500] for item in value.get("concerns") or [] if str(item).strip()][:12],
            "actions":["Apply exact phrase","Apply product brief","Apply listing title","Apply special instructions","Apply all"],"provider_calls_performed":False,"form_submitted":False}
        history["messages"].append({"created_at":datetime.now().astimezone().isoformat(),"user":message,"response":result["response"],"suggestions":suggestions})
