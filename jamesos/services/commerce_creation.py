...
class CommerceCreationService:
    ...
    def review_snapshot(self,job_id:str)->dict[str,Any]:
        ...
        return {"job_id":job_id,"ready_for_review":status["ready_for_review"],"selected_candidate_id":selected.get("candidate_id"),"generation_method":selected.get("generation_method") or "deterministic_local_typography","dimensions":selected.get("dimensions") or selected.get("canvas_dimensions") or [selected.get("width"),selected.get("height")],
            "brand_display_name":state.get("brand_display_name"),"printify_shop_title":destination.get("printify_shop_title"),"printify_product_id":draft.get("printify_product_id"),"listing_title":listing.get("title"),"description":listing.get("description"),"tags":tags,"artwork_palette":selected.get("palette_summary") or brief.get("artwork_palette") or [],"garment_colors":brief.get("garment_colors") or listing.get("colors") or [],"provider_contacted":status["provider_contacted"],"workflow_timeline":status["workflow_timeline"],"publication_status":status["publication_status"],"order_status":status["order_status"],"artwork_url":f"/commerce/jobs/{job_id}/artwork-preview" if selected.get("png_path") else None}
